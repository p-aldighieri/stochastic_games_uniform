...
Version~13 keeps the structural terminal fix and adds the
schedule-independence repair for the global regret comparison. Every former
terminal boundary label $b$ is replaced by a \emph{degenerate terminal node}
$T_b$, a leaf of the universal tree with a single internal absorbing state, no
exits, constant per-stage payoff vector $w(b)$, trivial bookkeeping kernel, and
exact Bellman certificate
\[
g_{T_b}=w(b),
\qquad
h_{T_b}\equiv 0.
...
Moreover, each launched block now has a publicly fixed scheduled boundary: once
its cap $L$ is announced by the routing automaton, that block keeps the interval
of length $L$ whether or not its live first-exit time is shorter. Thus terminal
behavior remains inside the same block system as every other node, no
post-terminal segment has to be estimated separately, the spurious $\Theta(T)$
horizon gap disappears, and the compliant and deviating payoffs are compared on
common launched block intervals.
...
\subsection{The v13 repair}

Version~13 keeps the v10-v12 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node, and
renewal-reward repairs, and adds a strategy-independent launched block
partition for the global regret comparison.
...
\item The local payoff identity is written in renewal-reward form on the live
...
state $\xi_C^\circ$ where $h_{i,C}^*(\xi_C^\circ)=0$. For the horizon-$T$
regret comparison, however, one does not repartition play according to the
random live times. One keeps the launched block partition fixed and compares the
compliant and deviating payoffs on those same scheduled block intervals.
...
number of live stages in $B$ at which $(X_t,A_t)=(\xi,a)$, and let $I_B(b)$ be
the indicator that the live segment of $B$ exits at $\partial_b$. The empirical
live-time rates are
\[
...
\]
The total augmented block reward recorded on the live part of $B$ is
\begin{equation}
...
\end{equation}
If the live segment reaches its cap without a cemetery hit, the second term is
zero. If $C=T$ is degenerate terminal, then $B_C=\varnothing$, so the second term
is always absent and
\[
\widetilde U_i(B)=\sum_{t\in B^{\mathrm{live}}}u_i^t
= w_i(T)\,\tau(B)
= w_i(T)\,|B|,
\]
...
label $b$ is recorded and the public memory remains in that absorbed intrablock
state until $t_k+L_k$.
...
already occurred. The launch times and block boundaries are therefore publicly
observable and deterministic conditional on that memory.
\end{enumerate}
\end{proposition}
...
stores the absorbed cemetery state until the already announced block boundary.
Because the first cemetery hit is unique, at most one boundary label can be
recorded.
...
interval itself. Therefore both the compliant and deviating augmented payoffs on
block $k$ are evaluated on the same launched interval $B_k$; within-block exits
are handled inside that fixed interval by the absorbed cemetery state.
\end{remark}
...
-
g_{i,C}^*\,\E\bigl[\tau(B)\mid \cF_{\mathrm{in}(B)}\bigr]
\right|
\le C_\eta\delta\,\E\bigl[\tau(B)\mid \cF_{\mathrm{in}(B)}\bigr];
\end{equation}
...
-
\frac1T\sum_{B\subseteq [1,T]} \tau(B)\,g_{i,C(B)}^*
\right|
\le C_\eta\delta + \frac{K_\eta}{T},
\end{equation}
where the sum is over complete scheduled blocks contained in $[1,T]$ and $C(B)$
denotes the node of block $B$ at launch. The constant $K_\eta$ absorbs the
frozen prelude and the initial and final incomplete fragments.
\end{enumerate}
\end{theorem}
...
applied to the live segment of tail blocks, again conditioned on the entry state
and with constants enlarged uniformly over the finite tree.

For (Q3), decompose the horizon into the complete scheduled blocks contained in
$[1,T]$ together with the initial and final incomplete fragments. Among the
complete blocks, only finitely many belong to the frozen prelude and have cap
$<M_*$. Their combined contribution to the realized payoff and to the weighted
gain sum is bounded by a finite constant depending only on the frozen prelude,
the tail cap $M_*$, and the finite tree object; absorb that constant into
$K_\eta$. Summing \eqref{eq:block-payoff-realization} over the remaining tail
blocks and using
\[
\sum_{B\subseteq [1,T]} \tau(B)\le T
\]
gives the weighted gain approximation.

It remains to compare augmented and ordinary payoffs. A complete scheduled block
differs from its ordinary stage-payoff sum only when its live segment exits to a
cemetery state, in which case the augmented payoff contains one continuation term
lying in $[0,D_\eta]$. By \Cref{prop:public-routing}, the play can contain at
most $D_\eta$ such recorded exits. Blocks played after the route has entered a
degenerate terminal node have $B_C=\varnothing$ and therefore carry no
continuation bonus at all. Hence the total augmented/ordinary discrepancy over
all complete scheduled blocks inside $[1,T]$ is at most $D_\eta^2$.

The incomplete fragments contribute at most a finite constant depending only on
the frozen prelude, the tail cap $M_*$, and the finite tree object. Absorbing
these constants into $K_\eta$ yields \eqref{eq:realization-tail-bound}.
\end{proof}
...
\begin{aligned}
\E\left[
\widetilde U_i(B;\tau_i,\sigma_{-i}) - g_{i,C}^*\tau(B)
\Bigm|\cF_{\mathrm{in}(B)}
\right]
&\le
(\eta+C_\eta\delta)\,\E\bigl[\tau(B)\mid \cF_{\mathrm{in}(B)}\bigr]
\\
&\quad+
...
\end{aligned}
\end{equation}
Here $\widetilde U_i(B;\tau_i,\sigma_{-i})$ denotes the augmented live reward when
player $i$ deviates inside the launched block $B$ and the others keep the
realized profile.
\end{lemma}
...
Let $\tau(B)$ be the live stopping time inside the launched block $B$. Define the
augmented one-step reward $\widetilde u_i^t(\tau_i,\sigma_{-i})$ along the live
segment so that it equals the ordinary stage payoff inside $\Xi_C$ and includes
the continuation term exactly at the live stage where the block first enters a
cemetery state. Summing the Bellman inequality from the block entry up to time
$\tau(B)$ and taking conditional expectations gives
\begin{align*}
...
The terminal bias term is zero on a cemetery exit by the extension convention
above, and it is the carried-over bias of the terminal internal state when the
live segment ends at its cap.
\end{proof}
...
\end{aligned}
\end{equation}
Both block payoffs in \eqref{eq:block-regret-ineq} are evaluated on the same
launched scheduled block interval $B$ from
\Cref{def:scheduled-block-partition}; only the live first-exit segment inside $B$
changes with the deviator's actions.
\end{corollary}

\begin{proof}
The key point is that $B$ denotes the launched block interval fixed by the public
routing automaton. By \Cref{rem:strategy-independent-blocks}, a unilateral
deviation may change only the live path inside $B$, not the interval $B$ itself.
Apply \Cref{lem:block-bellman} to the deviator's augmented live payoff on that
common block. By \eqref{eq:block-payoff-realization}, the compliant controller on
the same launched block satisfies
\[
...
complete blocks is bounded by $2D_\eta^2$. The incomplete initial and final
fragments contribute at most a finite constant depending only on the frozen
prelude, the tail cap, and the finite tree object. Absorb all of these terms
into a constant $K_\eta<\infty$.
...
order. By \Cref{rem:strategy-independent-blocks}, these are the common block
intervals on which both the compliant and deviating block payoffs are evaluated:
a deviation can change only the live first-exit path inside each $B_r$, not the
endpoints of $B_r$ itself. The finitely many prelude blocks, before the
...
$D_\eta$ times. Therefore \Cref{lem:aggregation} gives
\[
...
It remains to pass from augmented block payoffs to ordinary stage payoffs. A
complete scheduled block differs from its ordinary stage-payoff sum only if its
live segment exits to a cemetery state, in which case one continuation term in
$[0,D_\eta]$ is added. Under either the deviating or the compliant profile, the
path can contain at most $D_\eta$ such recorded exits. Hence the total difference
between augmented regret and ordinary regret over all complete scheduled blocks
in $[1,T]$ is at most $2D_\eta^2$, already absorbed into $K_\eta$. Blocks played
after entry into a degenerate terminal node have no cemetery exit and therefore
create no extra discrepancy. The incomplete fragments are already absorbed into
$K_\eta$. We obtain
\[
...
The central conceptual distinction in v13 is threefold: bookkeeping versus
execution, degenerate terminal nodes instead of terminal labels, and scheduled
block boundaries versus live first-exit times. The restart kernel $\rho_C$
belongs only to the convex bookkeeping package; it is not an executed transition
after a cemetery hit. Execution takes place on the genuine absorbing augmented
kernel, the live segment ends at the first cemetery state, and the relevant
local identity is renewal-reward: the bookkeeping rates determine $g_{i,C}^*$,
while the executed controller contributes that gain through total augmented live
reward divided by actual first-exit time. Version~13 adds a strategy-independent
launched block partition: the public routing automaton fixes each block interval
at launch, while the live first-exit time determines only what happens inside
that interval and which successor node is recorded for the next block. When a
block records a boundary label leading to a degenerate terminal node $T_b$, the
next scheduled block is simply played in $T_b$, whose gain is exactly $w(b)$ and
whose bias is zero. The block schedule therefore continues through terminal
behavior instead of stopping before the horizon analysis is finished, and the
global regret comparison is performed on common launched block intervals rather
than on profile-dependent first-exit partitions. With those distinctions in
place, the fixed-point domain is convex, the Bellman target matches the realized
live first-exit segment, and the routing semantics are well-defined.
...
$B_r$, let $\tau_r:=\tau(B_r)$ be its live duration, and let $\Delta_r$ be a
block contribution.
