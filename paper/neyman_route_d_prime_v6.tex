...
Version~6 repairs four blockers in version~5. First, the Kakutani domain is made
convex by replacing the non-linear normalization $\min h=0$ with the linear
reference-state normalization $h(\xi_C^\circ)=0$. Second, every statement that
uses irreducibility, powers of kernels, or hitting-time bounds is rewritten for the
\emph{genuine absorbing augmented kernel} on
$\Xi_C^{\mathrm{aug}}=\Xi_C\sqcup\partial B_C$, not for the sub-stochastic
internal kernel. Third, the local target payoff is aligned with the proof by
working with the \emph{augmented block payoff}: while the block stays in $\Xi_C$,
the payoff is the ordinary stage payoff; at the first visit to a cemetery state
$\partial_b$, the block records the continuation value $v_{i,C}(b)$; after that
the block is padded by zeros. Fourth, routing is made precise: a block terminates
at the first cemetery visit, emits at most one boundary label, and never
regenerates through cemetery states.
...
\[
\eta + C_\eta\delta + \frac{4H_\eta}{M_*} + \frac{K_\eta}{T}.
\]
...
equilibrium is mathematically correct once the v5 blockers are repaired.
\end{remark}

\subsection{The v6 repairs}

Version~6 keeps the earlier frozen-continuation and local-Nash repairs from v5
and adds four structural corrections that are essential for correctness.
...
$\Xi_C^{\mathrm{aug}}=\Xi_C\sqcup\partial B_C$, not for the sub-stochastic
internal kernel.
\item The local target payoff is the augmented block payoff, namely the average of
ordinary stage payoffs inside $\Xi_C$ plus the continuation value paid at the
first cemetery hit, if such a hit occurs.
\item A block at node $C$ terminates at its first cemetery visit. It emits at
most one boundary label, moves to the corresponding child node, and never
regenerates through cemetery states. The internal restart map $\rho_C$ is used
only in the bookkeeping occupation package.
\end{enumerate}
...
\]
\item A bookkeeping restart kernel
\[
...
$\sigma_C\in\Sigma_C$ and every block length $L\ge 1$, the universal
$\eta$-tree representation supplies a public finite-memory first-exit block
controller $\mathcal R_{C,L}^{\sigma_C}$ with the following properties.
\begin{enumerate}[label=\textnormal{(R\arabic*)},leftmargin=2.8em]
\item The block starts at an entry phase-state in $\Xi_C$, evolves on the genuine
augmented state space $\Xi_C^{\mathrm{aug}}=\Xi_C\sqcup\partial B_C$, and
terminates at the first visit to a cemetery state $\partial_b$ or at time $L$ if
no cemetery state is visited.
\item At most one boundary label is emitted. If the block hits $\partial_b$, that
unique label is $b$.
\item Let $\widehat\mu_{C,L}^{\mathrm{book}}$ be the normalized internal
state-action occupation of the controller up to block termination, padded by zeros
after termination, and let $\widehat\beta_{C,L}$ be the normalized first-exit
boundary indicator:
\[
\widehat\beta_{C,L}(b):=\frac1L\,\1_{\{\text{the first cemetery hit is }\partial_b\}}.
\]
Then, uniformly over entry states, there exists $K_C(\sigma_C)<\infty$ such that
\begin{equation}
\label{eq:controller-input-occ}
\E\Bigl[
\bigl\|(\widehat\mu_{C,L}^{\mathrm{book}},\widehat\beta_{C,L})
-
\bar\mu_C^\sigma
\bigr\|_1
\Bigr]
...
\end{equation}
\item For every continuation vector $v_C(\cdot)\in[0,1]^{B_C\times N}$, the
controller's augmented block payoff is
\[
\widetilde U_{i,C,L}
:=
\sum_{t=1}^{L} u_i^t\,\1_{\{X_t\in \Xi_C\}}
+
\sum_{b\in B_C}\1_{\{\text{the first cemetery hit is }\partial_b\}}\,v_{i,C}(b),
\]
where the process is padded by zeros after block termination.
\end{enumerate}
\end{assumption}
...
$\widetilde Q_C^{\sigma}$, define the internal occupation measure and exit mass by
\[
...
$v_C^x(b)$ is the continuation vector stored at $b$ in $x$ and required later to
be $\eta$-close to $g_D$; if $C[b]$ is a terminal label, then $v_C^x(b)$ is the
...
$x$; see \Cref{prop:mdp-duality}. Hence there exists an optimal stationary mixed
kernel together with a gain-bias pair satisfying
\eqref{eq:Bellman-ineq-def} and the support equalities. Therefore
$\mathfrak B_{i,C}(x)$ is nonempty.
...
with the linear normalization hyperplane $h(\xi_C^\circ)=0$, and the continuation
values lie in cubes. Because the tree is finite, $\cA_\eta$ is a finite product of
compact convex sets.
\end{proof}
...
$\mathcal R_{C,L}^{\sigma_C^*}$ supplied by \Cref{ass:controller-input}. Inside a
scheduled block of length $L$, the controller evolves on
$\Xi_C^{\mathrm{aug}}=\Xi_C\sqcup\partial B_C$ and stops at the first cemetery hit.

For a realized block $B$ in node $C$, let $|B|$ denote its scheduled length and
let $\widetilde U_i(B)$ denote the augmented block payoff:
\begin{equation}
\label{eq:aug-block-payoff}
\widetilde U_i(B)
:=
\sum_{t\in B} u_i^t\,\1_{\{X_t\in \Xi_C\}}
+
\sum_{b\in B_C}\1_{\{\text{the first cemetery hit in }B\text{ is }\partial_b\}}
\,v_{i,C}^{x^*}(b).
\end{equation}
After the first cemetery hit the block is padded by zeros, so \eqref{eq:aug-block-payoff}
contains at most one continuation term.
...
$\sigma_C^*$. Then there exists a constant $K_C<\infty$ such that for every block
length $L\ge 1$, every entry phase-state, and every player $i$,
\begin{equation}
\label{eq:local-controller-occ}
\E\bigl[\|(\widehat\mu_{C,L}^{\mathrm{book}},\widehat\beta_{C,L})-\bar\mu_C^*\|_1\bigr]
\le \frac{K_C}{\sqrt L},
\end{equation}
and
\begin{equation}
...
\left|
\E\Bigl[\frac{\widetilde U_{i,C,L}}{L}\Bigr]-g_{i,C}^*
\right|
\le \frac{K_C}{\sqrt L}.
\end{equation}
...
Now define the bounded linear functional
\[
\Phi_{i,C}(\mu,\beta)
:=
...
\]
By \Cref{ass:controller-input}, the normalized augmented block payoff is precisely
$\Phi_{i,C}(\widehat\mu_{C,L}^{\mathrm{book}},\widehat\beta_{C,L})$. Hence
\[
\left|
\E\Bigl[\frac{\widetilde U_{i,C,L}}{L}\Bigr]-\Phi_{i,C}(\bar\mu_C^*)
\right|
\le
\|\Phi_{i,C}\|_{\mathrm{Lip}}\,
\E\bigl[\|(\widehat\mu_{C,L}^{\mathrm{book}},\widehat\beta_{C,L})-\bar\mu_C^*\|_1\bigr]
\le \frac{K_C'}{\sqrt L}.
\]

It remains to identify the target. Integrating the Bellman equalities for actions
in the support of $\sigma_{i,C}^*(\cdot\mid\xi)$ against the bookkeeping package
$\bar\mu_C^*=(\mu_C^*,\beta_C^*)$ yields
\[
g_{i,C}^*
=
...
properties.
\begin{enumerate}[label=\textnormal{(R\arabic*)},leftmargin=2.8em]
\item At each scheduled block played in node $C$, the profile runs the first-exit
controller $\mathcal R_{C,|B|}^{\sigma_C^*}$ from \Cref{thm:local-controller}.
\item A block terminates at its first visit to a cemetery state $\partial_b$, if
such a visit occurs, and otherwise at the scheduled time $|B|$.
\item If a block terminates at $\partial_b$, the next block starts in the child
node $C[b]$. If no cemetery state is visited, the next block remains in node $C$.
\item After a cemetery hit, the remainder of the scheduled block time is padded by
zeros, contributes no further payoff, and causes no further state change inside the
block. Hence every block emits at most one boundary label.
\item The public memory stores the current node, the current block index, and the
current local controller state. The routing update is publicly observable and
deterministic conditional on that information and on the unique boundary label, if
one is emitted.
\end{enumerate}
\end{proposition}
...
data, so once a label $b$ is observed the next node is deterministic. If no label
is emitted, the controller remains in the same node. The memory requirements are
finite by \Cref{lem:frozen-memory}, and all relevant quantities are publicly
observable.
\end{proof}
...
\E\bigl[\|(\widehat\mu_B^{\mathrm{book}},\widehat\beta_B)-\bar\mu_C^*\|_1
...
\left|
\E\Bigl[\frac{\widetilde U_i(B)}{|B|}
\Bigm|\,\cF_{\mathrm{in}(B)}\Bigr]-g_{i,C}^*
\right|
\le C_\eta\delta;
\end{equation}
\item for every player $i$ and every complete block $B$,
\begin{equation}
\label{eq:block-stage-vs-aug}
\left|
\E\Bigl[\frac1{|B|}\sum_{t\in B}u_i^t
\Bigm|\,\cF_{\mathrm{in}(B)}\Bigr]
-
\E\Bigl[\frac{\widetilde U_i(B)}{|B|}
\Bigm|\,\cF_{\mathrm{in}(B)}\Bigr]
\right|
...
\right|
\le C_\eta\delta + \frac{1}{M_*} + \frac{K_\eta}{T},
\end{equation}
...
\begin{proof}
Item~(Q1) is \Cref{thm:local-controller} at the chosen tail scale. Since the
augmented payoff is the bounded linear functional $\Phi_{i,C}$ from the proof of
\Cref{thm:local-controller}, (Q2) follows from (Q1) after multiplying by a
uniform Lipschitz constant depending only on the finite tree object.

Item~(Q3) holds because each block records at most one continuation bonus and each
continuation value lies in $[0,1]$. Therefore the augmented block payoff and the
ordinary block stage payoff differ by at most $1$ in absolute value, hence by at
most $1/|B|$ after normalization.

For (Q4), decompose the horizon into the complete blocks contained in $[1,T]$ plus
at most one initial incomplete fragment and one final incomplete fragment. Summing
(Q2) over the complete blocks gives the augmented-payoff approximation. Then use
(Q3) and the fact that every complete block has length at least $M_*$, so the
total stage/augmented discrepancy over complete blocks is at most
\[
\frac1T\sum_{B\subseteq[1,T]} 1
\le \frac1{M_*}.
\]
The incomplete fragments contribute at most $K_\eta/T$ because the frozen prelude
is finite and stage payoffs are bounded by $1$. This yields
\eqref{eq:realization-tail-bound}.
\end{proof}
...
\begin{proof}
Set $m:=|X|-1$. By definition of $\kappa(P)$,
\[
\Pb_x(\tau_{A_y}\le m)\ge \Pb_x(X_m=y)\ge \kappa(P).
\]
Indeed the event $\{X_m=y\}$ already excludes prior absorption because all states in
$\partial$ are absorbing. Therefore
\[
...
\[
\|h_{i,C}^*\|_\infty\le H_\eta:=1+\frac{\max_D|\Xi_D|-1}{\kappa_\eta}.
\]
In particular,
\[
...
\right].
\]
Since $u_i^t,g_{i,C}^*,v_{i,C}^{x^*}(b)\in[0,1]$, the absolute value of the last
display is bounded by
\[
\E_\xi[\tau^\circ] + 1
\le
1+\frac{|\Xi_C|-1}{\kappa_\eta}
\le
1+\frac{\max_D|\Xi_D|-1}{\kappa_\eta}
=
H_\eta.
\]
...
stationary bookkeeping package $\bar\mu_C^*$, and the block payoff is the
augmented payoff from \eqref{eq:aug-block-payoff}.
...
\E\left[
\frac{\widetilde U_i(B;\tau_i,\sigma_{-i})}{|B|}
...
\nonumber
\end{align}
The realized compliant controller is $\delta$-close to the fixed-point bookkeeping
package by \eqref{eq:block-occupation-realization}. Since the coefficient arrays of
the deviation MDP are Lipschitz in the compliant kernel,
\Cref{lem:deviation-lipschitz} implies that the one-step coefficients seen by the
deviator differ from the fixed coefficients in
\eqref{eq:exact-bellman-fixed-point} by at most $C_\eta\delta$ in conditional
expectation, for a constant depending only on the finite tree object and $H_\eta$.

Therefore the process
\[
M_t:=h_{i,C}^*(X_t)-\sum_{s=t_0}^{t-1}
\bigl(\widetilde u_i^s(\tau_i,\sigma_{-i})-g_{i,C}^*-\eta-C_\eta\delta\bigr)
\]
has nonnegative conditional drift along the block, where
$\widetilde u_i^s(\tau_i,\sigma_{-i})$ denotes the augmented one-step payoff that
equals the ordinary stage payoff inside $\Xi_C$ and the continuation value
$v_{i,C}^{x^*}(b)$ if the first cemetery hit at step $s$ is $\partial_b$. Summing
from the block entry $t_0$ to the block exit $t_1-1$ and taking expectations
yields
\[
...
$2\|h_{i,C}^*\|_\infty\le 2H_\eta$ by \Cref{prop:bias-span}. Dividing by $|B|$
gives \eqref{eq:block-bellman-ineq}.
\end{proof}
...
\right]
\le
\eta + C_\eta\delta + \frac{4H_\eta}{|B|}.
\]
\end{corollary}

\begin{proof}
Apply \Cref{lem:block-bellman} to the deviator's augmented block payoff. By
\eqref{eq:block-stage-vs-aug}, replacing augmented payoffs by ordinary stage
payoffs changes either player's block average by at most $1/|B|$. Since
$H_\eta\ge 1$ by \Cref{prop:bias-span}, the two replacement errors contribute at
most $2/|B|\le 2H_\eta/|B|$. Together with the Bellman term $2H_\eta/|B|$ from
\Cref{lem:block-bellman} and the compliant approximation
\eqref{eq:block-payoff-realization}, this gives the stated bound after enlarging
$C_\eta$ if necessary.
\end{proof}
...
\[
H_\eta:=1+\frac{\max_D|\Xi_D|-1}{\kappa_\eta}.
\]
...
$\tau_i$. Partition the horizon $\{1,\dots,T\}$ into the complete blocks contained
in $[1,T]$ plus the incomplete initial and terminal fragments. Applying
...
The central conceptual distinction in v6 is the separation between bookkeeping and
...
cemetery state, and the augmented block payoff includes the continuation value only
once. With that distinction in place, the fixed-point domain is convex, the
Bellman target matches the realized block payoff, and the routing semantics are
well-defined.
