...
Version~21 fixes the remaining defect in v20. The raw nodewise bias potential
\[
\Phi_{i,t}:=h_{i,C_t}^*(X_t)
\]
is not continuous across strict node changes because the bias functions depend on
the current node. The correct telescope variable is the stock-augmented
potential
\[
\Psi_{i,t}:=h_{i,C_t}^*(X_t)+Z_{i,t},
\]
where $Z_{i,t}$ starts at zero and, whenever stage $t$ exits node $C$ through
boundary $b$ into $D=C[b]$, receives the increment
$v_{i,C}^{x^*}(b)-h_{i,D}^*(X_{t+1})$. This stock absorbs exactly the mismatch
between the Bellman exit credit in $C$ and the successor entry bias in $D$.

With this definition the one-step Bellman drift is clean at every stage:
\[
\E\bigl[u_i(s_t,a_t)\mid \cF_t\bigr]
=
g_{i,C_t}^*
+
\Psi_{i,t}
-
\E[\Psi_{i,t+1}\mid \cF_t]
+
O(\eta+\delta),
\]
with equality on the support of the compliant local kernel and the corresponding
upper-bound inequality for a unilateral deviator. Because every stock jump is
uniformly bounded and the routed tree allows only finitely many strict child
moves along a play path, $\Psi$ is uniformly bounded. Summing the drift over the
horizon yields
\[
\E\Bigl[\sum_{t=1}^T u_i(s_t,a_t)\Bigr]
=
\E\Bigl[\sum_{t=1}^T g_{i,C_t}^*\Bigr]
+
\E[\Psi_{i,1}-\Psi_{i,T+1}]
+
O\!\left((\eta+\delta)T\right),
\]
up to the usual finite prelude and end-fragment bookkeeping terms.

For the compliant profile, the stagewise gain sum is identified with a rooted
semi-Markov benchmark $\Gamma_{i,T}(s;x^*)$. For a unilateral deviator, the same
augmented potential telescope gives an upper bound because any move toward a
node with larger local gain is offset by the drop encoded by the continuation
constraint. The resulting regret bound remains of order
\[
...
\subsection{The v21 repair}

Version~21 keeps the v10--v18 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
expectation-bookkeeping, finite-memory, and one-profile Bellman repairs, keeps
the v20 stagewise architecture, and fixes the remaining defect in the v20
telescope variable.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The defect is now stated plainly. The visit functional
\[
...
\item The missing object is an explicit transition stock. Define
\[
\Psi_{i,t}:=h_{i,C_t}^*(X_t)+Z_{i,t},
\qquad
Z_{i,1}=0,
...
\item The correct proof is stagewise with the augmented potential. The Bellman
certificate gives the one-step drift formula
\[
\E\bigl[u_i(s_t,a_t)\mid \cF_t\bigr]
=
g_{i,C_t}^*
...
\]
with equality on the support of the compliant local kernel and the corresponding
upper-bound inequality for a deviator. There is no leftover boundary mismatch:
it has been moved into $Z$.

\item Summing over the horizon yields
\[
\E\Bigl[\sum_{t=1}^T u_i(s_t,a_t)\Bigr]
=
\E\Bigl[\sum_{t=1}^T g_{i,C_t}^*\Bigr]
...
O\!\left((\eta+\delta)T\right).
\]
The endpoint term is harmless because $\Psi$ is uniformly bounded. In the
present routed tree one even has a pathwise bound, since there are at most
$D_\eta$ strict child moves.
...
\item For a unilateral deviator, the same augmented potential still telescopes.
If the deviator steers play toward a node with larger local gain, the resulting
gain increase is offset by the drop encoded by the continuation constraint
\eqref{eq:frozen-continuation}. Thus the potential-adjusted payoff is bounded by
the same rooted benchmark.

\item Continuation values remain exactly where they belong: in the one-step
Bellman equations, in the explicit transition stock $Z$, and in the fixed-point
continuation constraint. They are not inserted as literal stage payoffs after an
exit, and they are not telescoped visit by visit.
\end{enumerate}
...
Version~21 keeps it local and performs the global telescope instead with the
...
The correct global argument is stagewise with an explicit transition stock. Fix a
fixed point $x^*\in\BR_\eta(x^*)$, a player $i$, a profile $\varsigma$ following
the public visit-routing convention, and a horizon $T$. Let
\[
\mathcal T_T(\varsigma)\subset\{1,\dots,T\}
\]
be the set of stochastic-game stages belonging to complete tail visits contained
in $[1,T]$. For every live stage $t$, let $C_t=C_t(\varsigma)$ be the current
node and let $X_t=X_t(\varsigma)\in\Xi_{C_t}$ be the current phase-state. The raw
nodewise bias term is
\[
\Phi_{i,t}(\varsigma):=h_{i,C_t}^*(X_t).
\]
This quantity alone does not telescope across strict node changes because the
bias functions depend on the current node.
...
stock $Z$ is what repairs the boundary mismatch between a parent-node Bellman
credit $v_{i,C}^{x^*}(b)$ and the successor-node entry bias.
\end{remark}
...
the deviator's stopping rule. Whenever a strict exit sends play from $C$ to
$D=C[b]$, the fixed-point continuation relation \eqref{eq:frozen-continuation}
gives
\[
g_{i,D}^*\le v_{i,C}^{x^*}(b)+\eta.
\]
Since the augmented potential jumps to the value
$Z_{i,t}+v_{i,C}^{x^*}(b)$ at that transition, the potential-adjusted gain process
\[
M_t
:=
\sum_{s\le t,\ s\in\mathcal T_T(\varsigma^\tau)} g_{i,C_s}^*
+
\Psi_{i,t}(\varsigma^\tau)
\]
is a supermartingale up to $O(\eta)$-drift along the routed tail chain. Summing
that drift over the tail stages and using the uniform bound
\eqref{eq:psi-bound} on the endpoint terms yields
\eqref{eq:rooted-benchmark-deviator}.
\end{proof}
...
$\Gamma_{i,T}(s;x^*)$. Version~21 uses that object only to identify the gain sum
$\mathfrak G_{i,T}$; the actual payoff comparison itself is stagewise and driven
by the stock-augmented Bellman potential.
\end{remark}
...
remainder from \Cref{prop:deviation-total}, the endpoint term of the stock-augmented potential from
...
The conceptual point of v21 is that the global telescope is \emph{not}
visitwise, and it is not based on the raw nodewise bias alone. The v19 auxiliary
functional
\[
\widetilde U_i(V)
=
...
\]
credits a strict exit but does not debit the successor-node entry bias. The v20
rewrite already recognized that defect, but it still suppressed the stock needed
to make the stagewise potential continuous across node changes.

The corrected v21 proof introduces that stock explicitly:
\[
\Psi_{i,t}
=
h_{i,C_t}^*(X_t)+Z_{i,t},
...
v_{i,C_t}^{x^*}(b)-h_{i,C_t[b]}^*(X_{t+1})
\]
at a strict exit through $b$. With this definition the Bellman drift is clean:
\[
\E\bigl[u_i(s_t,a_t)\mid \cF_t\bigr]
=
g_{i,C_t}^*
...
\E[\Psi_{i,t+1}\mid \cF_t]
\]
for compliant play, with the corresponding upper-bound inequality for a
deviator. The stock process absorbs exactly the parent/child mismatch at the
boundary, so after summing over the horizon only the bounded endpoint values of
$\Psi$ remain.
...
the same augmented potential drift yields an upper bound because any attempt to
steer play toward temporarily larger node gains is paid for by a compensating
drop through the continuation constraint \eqref{eq:frozen-continuation}.

In short, v21 keeps continuation values inside the Bellman drift where they are
legitimate, makes the carried stock explicit, and compares payoffs by a genuine
stagewise telescope rather than by any visitwise chaining of
$\widetilde U_i$.
\appendix
