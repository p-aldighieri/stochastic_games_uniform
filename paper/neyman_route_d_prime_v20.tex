...
\title{\textbf{Uniform Equilibrium from an Augmented Metastable Tree\\via a Stage-by-Stage Potential Telescope}}
...
Version~20 replaces the invalid v19 global telescope. The auxiliary visit
functional
\[
\widetilde U_i(V)
=
...
\]
credits strict exits but has no matching entry debit in the successor visit:
the child visit starts with the child-node bias
$h_{i,D}^*(X_{\mathrm{in}})$, not with $-v_{i,C}(b)$. Hence
$\widetilde U_i$ cannot be telescoped visit by visit.

The correct proof is stagewise. Let $C_t$ be the current routed node and $X_t$
the current phase-state. The one-step Bellman certificate gives
\[
u_i(s_t,a_t)
=
g_{i,C_t}^*
...
\Phi_{i,t}
-
\E[\Phi_{i,t+1}\mid \cF_t]
+
O(\eta+\delta),
\qquad
\Phi_{i,t}:=h_{i,C_t}^*(X_t),
\]
up to a uniformly bounded strict-exit correction carried by the public routing
state. Summing over the horizon yields an $O(H_\eta/T)$ boundary term. For the
compliant profile, the stagewise gain sum is identified with a rooted
semi-Markov benchmark $\Gamma_{i,T}(s;x^*)$. For a unilateral deviator, the same
potential telescope gives an upper bound because any move toward a node with
larger local gain is offset by the drop of the Bellman potential encoded by the
continuation constraint. The resulting regret bound is of order
\[
...
\subsection{The v20 repair}

Version~20 keeps the v10--v18 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
expectation-bookkeeping, finite-memory, and one-profile Bellman repairs, but it
replaces the fatal v19 global step.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The defect in v19 is now stated plainly. The visit functional
\[
\widetilde U_i(V)=U_i^{\mathrm{act}}(V)+\sum_{b\in B_C}I_V(b)\,v_{i,C}(b)
\]
credits exits but does not subtract the successor-node entry bias. If a visit in
node $C$ exits through $b$ and the next visit starts in $D=C[b]$, then the next
visit begins with $h_{i,D}^*(X_{\mathrm{in}})$, not with $-v_{i,C}(b)$. Hence
there is no legitimate cross-visit telescoping of $\widetilde U_i$.

\item The correct proof is stagewise. Let $C_t$ be the current node and $X_t$ the
current phase-state at stage $t$. The Bellman certificate gives the one-step
drift formula
\[
u_i(s_t,a_t)
=
...
\item Summing over $T$ stages yields
\[
\sum_{t=1}^T u_i(s_t,a_t)
=
\sum_{t=1}^T g_{i,C_t}^*
+
\Phi_{i,1}
...
\E[\Phi_{i,T+1}]
+
O\!\left((\eta+\delta)T\right)
+
O_\eta(1).
\]
Since $\|\Phi\|_\infty\le H_\eta$, the boundary contribution becomes
$O_\eta(1/T)$ after normalization by $T$.

\item For the compliant profile, the stagewise gain sum is identified with a
rooted semi-Markov benchmark $\Gamma_{i,T}(s;x^*)$ by the stationary visit law
induced by condition~\textnormal{(a)}.

\item For a unilateral deviator, the same potential still telescopes. If the
deviator steers play toward a node with larger local gain, the Bellman potential
drops by the matching continuation amount encoded by
\eqref{eq:frozen-continuation}. Thus the potential-adjusted payoff is bounded by
the same rooted benchmark.

\item Continuation values remain where they belong: in the one-step Bellman
equations and in the fixed-point continuation constraint. They are not inserted as
literal stage payoffs after an exit, and they are not telescoped visit by visit.
\end{enumerate}
...
stagewise potential drift lemma, the rooted stagewise benchmark lemma, the
compliant total benchmark, the deviator total benchmark, and finally
\Cref{thm:main}. The appendices record the finite-MDP facts and the stagewise
summation lemma.
...
Version~20 does not telescope it across visits, and it is not part of the payoff
criterion of the stochastic game.
...
\subsection{Stage-by-stage potential telescope and total benchmark assembly}
...
$\widetilde U_i$ across visits.

The correct global argument is stagewise. Fix a fixed point
$x^*\in\BR_\eta(x^*)$, a player $i$, a profile $\varsigma$ following the public
visit-routing convention, and a horizon $T$. Let
\[
\mathcal T_T(\varsigma)\subset\{1,\dots,T\}
\]
be the set of stochastic-game stages belonging to complete tail visits contained
in $[1,T]$. For $t\in\mathcal T_T(\varsigma)$, let $C_t=C_t(\varsigma)$ be the
current node and let $X_t=X_t(\varsigma)\in\Xi_{C_t}$ be the current phase-state.
Write
\[
\Phi_{i,t}(\varsigma):=h_{i,C_t}^*(X_t).
\]
Strictly speaking, at a strict exit one may add a single carried continuation
stock $Z_{i,t}$ which records the Bellman continuation credit created at the exit
and debits it at the successor entry. Because at most $D_\eta$ strict node changes
can occur along any play path and every bias is bounded by $H_\eta$, the total
variation of this stock is uniformly $O_\eta(1)$. We therefore suppress $Z_{i,t}$
in the notation and absorb it into the final endpoint constant.
...
node $C=C_t$, with current phase-state $\xi=X_t$. The Bellman certificate at the
fixed point gives, for every action $a_i\in A_i(\xi)$,
\[
u_i^t
\le
g_{i,C}^*
...
h_{i,C}^*(\xi)
-
\E\bigl[h_{i,C}^*(X_{t+1}^{\mathrm{loc}})\mid \cF_t\bigr]
-
\E\!\left[
\sum_{b\in B_C}\1_{\{\textnormal{exit through }b\textnormal{ at }t\}}
\,v_{i,C}^{x^*}(b)
\Bigm|\cF_t
\right]
+\eta,
\]
where $X_{t+1}^{\mathrm{loc}}$ denotes the next internal state when the current
visit stays in node $C$. On a strict exit, the Bellman term carries the frozen
continuation value $v_{i,C}^{x^*}(b)$ whereas the true successor stage starts from
the child-node bias $h_{i,C[b]}^*(X_{t+1})$. The mismatch is exactly the carried
continuation stock described above. Its total contribution is uniformly bounded by
$K_\eta^{\mathrm{pot}}$ because there are at most $D_\eta$ strict exits and every
bias is bounded by $H_\eta$.

Under the compliant profile, the controller plays $\sigma_C^*$ exactly at each
live stage, so the Bellman equalities hold on the support. Summing those
equalities over
$t\in\mathcal T_T(\sigma_{\eta,\delta,M_*})$, absorbing the bounded entry/exit
stock into $K_\eta^{\mathrm{pot}}$, and using the realization error from
\Cref{thm:realization}\textnormal{(Q1)--(Q2)} gives
\eqref{eq:stagewise-potential-compliant}.

Under a unilateral deviation, the nondeviators still generate the exact one-step
coefficients of the fixed-point Bellman problem. Summing the Bellman inequalities
over $t\in\mathcal T_T(\varsigma^\tau)$, telescoping the stagewise potential, and
absorbing the carried-stock remainder into $K_\eta^{\mathrm{pot}}$ gives
\eqref{eq:stagewise-potential-deviator}.
\end{proof}

\begin{lemma}[Stagewise rooted benchmark]
...
\Gamma_{i,T}(s;x^*)
\]
and finite constants $A_\eta^{\mathrm{sm}},K_\eta^{\mathrm{sm}}<\infty$ such that:
\begin{enumerate}[label=\textnormal{(\alph*)},leftmargin=2.8em]
\item for the compliant profile $\sigma_{\eta,\delta,M_*}$,
\begin{equation}
\label{eq:rooted-benchmark-compliant}
\E\bigl[\mathfrak G_{i,T}(\sigma_{\eta,\delta,M_*})\bigr]
\ge
\Gamma_{i,T}(s;x^*)
-
A_\eta^{\mathrm{sm}}\eta\,
...
$\varsigma^\tau:=(\tau_i,(\sigma_{\eta,\delta,M_*})_{-i})$,
\begin{equation}
\label{eq:rooted-benchmark-deviator}
\E\bigl[\mathfrak G_{i,T}(\varsigma^\tau)\bigr]
\le
\Gamma_{i,T}(s;x^*)
+
...
\begin{proof}
Define $\Gamma_{i,T}(s;x^*)$ to be the rooted stagewise benchmark generated by the
fixed-point routed tail process from initial state $s$. Under the compliant
profile, the sequence of visited nodes together with their sojourn lengths is the
finite semi-Markov process induced by the frozen realization. Condition
\textnormal{(a)} gives the communicating hypothesis on the augmented node kernels,
so the semi-Markov ergodic theorem identifies the compliant stagewise gain sum
with the rooted benchmark up to the $O(\eta)$ slack already present in the fixed
continuation relation and an $O_\eta(1)$ endpoint remainder. This proves
\eqref{eq:rooted-benchmark-compliant}.

For a unilateral deviator, one tracks the same routed node process but now under
the deviator's stopping rule. The fixed-point continuation relation
\eqref{eq:frozen-continuation} implies that when the deviator steers play toward a
child node with larger local gain, the Bellman potential drops by the matching
continuation amount up to $O(\eta)$. Equivalently, the potential-adjusted gain
process
\[
M_t:=\sum_{s\le t,\ s\in\mathcal T_T(\varsigma^\tau)} g_{i,C_s}^*
+
\Phi_{i,t}(\varsigma^\tau)
\]
is a supermartingale up to $O(\eta)$-drift. Summing this drift over the tail
stages and absorbing the bounded strict-exit corrections into
$K_\eta^{\mathrm{sm}}$ yields \eqref{eq:rooted-benchmark-deviator}.
\end{proof}

Fix once and for all a constant
\[
A_\eta\ge 1+A_\eta^{\mathrm{pot}}+A_\eta^{\mathrm{sm}}.
\]
...
\right].
\]
Apply \eqref{eq:stagewise-potential-compliant} from
\Cref{lem:visit-bellman} and then \eqref{eq:rooted-benchmark-compliant} from
\Cref{lem:tree-telescoping}. Absorb
$K_\eta^{\mathrm{pot}}+K_\eta^{\mathrm{sm}}$ into $K_\eta^{\mathrm{bal}}$.
\end{proof}

\begin{remark}[Semi-Markov viewpoint]
Under the compliant profile, the sequence of routed nodes together with their
sojourn lengths forms the semi-Markov object that produces the rooted benchmark
$\Gamma_{i,T}(s;x^*)$. Version~20 uses that object only to identify the gain sum
$\mathfrak G_{i,T}$; the actual payoff comparison itself is stagewise and driven
by the Bellman potential.
\end{remark}
...
\right].
\]
Apply \eqref{eq:stagewise-potential-deviator} from
\Cref{lem:visit-bellman} and then \eqref{eq:rooted-benchmark-deviator} from
\Cref{lem:tree-telescoping}. Since $C_\eta\delta\,\E[L_T(\varsigma^\tau)]$ is
nonnegative, it may be added harmlessly. Absorb
$K_\eta^{\mathrm{pot}}+K_\eta^{\mathrm{sm}}$ into $K_\eta^{\mathrm{dev}}$.
\end{proof}
...
remainder from \Cref{prop:deviation-total}, the bounded strict-exit stock from
\Cref{lem:visit-bellman}, the rooted semi-Markov benchmark remainder from
...
under condition~\textnormal{(a)} and the stagewise benchmark from
\Cref{lem:tree-telescoping}.

The conceptual point of v20 is that the global telescope is \emph{not}
visitwise. The v19 auxiliary functional
\[
\widetilde U_i(V)
=
U_i^{\mathrm{act}}(V)
+
\sum_{b\in B_{C(V)}}I_V(b)\,v_{i,C(V)}^{x^*}(b)
\]
credits a strict exit but does not debit the successor-node entry bias. The next
visit begins with the child bias $h_{i,D}^*(X_{\mathrm{in}})$, so the alleged
cross-visit cancellation is invalid.

The corrected proof instead telescopes the Bellman bias stage by stage:
\[
u_i(s_t,a_t)
=
g_{i,C_t}^*
...
\Phi_{i,t}=h_{i,C_t}^*(X_t),
\]
up to the uniformly bounded carried continuation stock created at strict exits.
After summing over the horizon, only the bounded initial and terminal potential
terms survive.

This is the standard potential argument for a Markov decision process, executed on
the routed node process. Under compliant play, the stagewise gain sum is the
semi-Markov benchmark attached to the fixed point. Under unilateral deviation,
the same Bellman drift yields an upper bound because any attempt to steer play
toward temporarily larger node gains is paid for by a compensating drop in the
Bellman potential through the continuation constraint
\eqref{eq:frozen-continuation}.

In short, v20 keeps continuation values inside the Bellman drift where they are
legitimate, and it compares payoffs by a stagewise potential telescope rather
than by a visitwise chaining of $\widetilde U_i$.
\appendix
