...
\title{\textbf{Uniform Equilibrium from an Augmented Metastable Tree\\via Chained Exit-Corrected Telescoping and Node-Visit Decomposition}}
...
Version~19 repairs the fatal v18 global-assembly error. The claim that the local
gains $g_{i,C}^*$ are nearly constant across nodes is false: a root node with
stage payoff $1$ and a terminal child with stage payoff $0$ already gives
$g_{i,\mathrm{root}}^*=1$ and $g_{i,\mathrm{child}}^*=0$. The proof therefore
does \emph{not} replace every node gain by a single root gain. Actual play still
stays in the genuine stochastic-game payoff model
\[
u_i(s_t,a_t)\in[0,1].
\]
Continuation values $v_C(b)$ are used only inside the fixed-point constraints and
the stopped Bellman certificates. They are never inserted as literal per-stage
payoffs after an exit. When a node visit exits through boundary label $b$, public
routing moves immediately to the successor node $C[b]$ and actual play resumes
there at the next stage.

The horizon-$T$ payoff is decomposed into a sum of actual payoffs over
\emph{node visits}, namely the contiguous stage intervals during which the public
routing automaton stays in a fixed node. For a visit $V\subset C$ we keep the
auxiliary exit-corrected functional
\[
\widetilde U_i(V)
=
U_i^{\mathrm{act}}(V)
+
\sum_{b\in B_C}I_V(b)\,v_{i,C}(b),
\]
but the global step is now to sum $\widetilde U_i$ over consecutive visits. The
boundary continuation generated at the exit of one visit is exactly the
continuation datum fed into the Bellman problem of the next visit. Hence the
relevant chained object is not $g_{i,\mathrm{root}}^*T$, but the rooted
gain-minus-boundary quantity
\[
\sum_V g_{i,C(V)}^*\,\ell(V)
-
\sum_V\sum_{b\in B_{C(V)}} I_V(b)\,v_{i,C(V)}(b),
\]
whose intermediate continuation terms telescope through the tree and leave only
$O(\|h\|_\infty)$ endpoint residuals.

For the compliant profile, the local-controller identities give a lower bound on
the total $\widetilde U_i$. For a unilateral deviator, the visitwise Bellman
inequality plus the same chained accounting gives an upper bound on the total
actual payoff along the deviator's own visit family. Both profiles are therefore
compared with the same rooted telescoping benchmark $\Gamma_{i,T}(s;x^*)$, with
errors of order
\[
A_\eta\eta + C_\eta\delta + \frac{K_\eta}{T}.
\]
Choosing the parameters in that order proves the theorem.
\end{abstract}
...
\subsection{The v19 repair}

Version~19 keeps the v10-v18 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
expectation-bookkeeping, finite-memory, and one-profile Bellman repairs, and it
replaces the fatal v18 global-assembly step.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The proof still treats the compliant profile and every unilateral
deviation separately. No argument matches a deviating visit with a compliant
visit one by one.
\item The v18 claim of \emph{gain near-constancy} is discarded. It is false in
general: if the root has stage payoff $1$ and its unique child is terminal with
payoff $0$, then
\[
g_{i,\mathrm{root}}^*=1,
\qquad
g_{i,\mathrm{child}}^*=0.
\]
Thus the node gains may vary by $O(1)$ across the tree.
\item The correct global object is the exit-corrected visit functional
\[
\widetilde U_i(V)=U_i^{\mathrm{act}}(V)+\sum_{b\in B_C} I_V(b)\,v_{i,C}(b),
\]
summed over \emph{consecutive} visits. Writing the visit family in chronological
order, the boundary continuation created at the exit of visit $r$ is exactly the
continuation datum used at the start of visit $r+1$. Hence the boundary chain
telescopes through the rooted tree, up to the bounded entry and terminal
endpoint terms coming from the Bellman biases.
\item The node-dependent gain terms are kept node-dependent. One does not try to
prove
\[
\sum_V g_{i,C(V)}^*\,\E[\ell(V)]
=
g_{i,\mathrm{root}}^*\,T.
\]
Instead one compares both the compliant and deviating visit families with the
same rooted chained benchmark
\[
\Gamma_{i,T}(s;x^*),
\]
obtained from the telescoping quantity
\[
\sum_V g_{i,C(V)}^*\,\ell(V)
-
\sum_V\sum_{b\in B_{C(V)}} I_V(b)\,v_{i,C(V)}(b).
\]
The fixed-point continuation constraints are exactly what make the intermediate
gain differences disappear in this rooted chain.
\item The stochastic-game payoff criterion remains the original horizon average
of actual stage payoffs. Continuation values are still used only inside the
stopped Bellman equations and the chained bookkeeping identities.
\item Because the rooted tree has finite depth, the total auxiliary boundary
chain, the Bellman endpoint remainder, the prelude visits before the frozen tail
cap, and the final incomplete fragment are all $O_\eta(1)$. The final regret
estimate is therefore again of order
\[
A_\eta\eta + C_\eta\delta + \frac{K_\eta}{T}.
\]
\end{enumerate}
...
tree-telescoping benchmark lemma, the compliant total benchmark, the visitwise
Bellman bound, the deviator total benchmark, and finally \Cref{thm:main}. The
appendices record the finite-MDP facts and the final visit aggregation
bookkeeping.
...
\end{equation}
This auxiliary quantity is used only in the stopped Bellman comparison and in
the expectation-level routing balance. It is not part of the payoff criterion
...
\subsection{Chained exit-corrected telescoping and total benchmark assembly}

The fatal v18 step was the attempt to replace every node-dependent benchmark
$g_{i,C(V)}^*\,\ell(V)$ by a single root gain. That is false in general. The
correct global object keeps the gains node-dependent and subtracts the chained
boundary continuation terms generated at strict exits.

Fix a fixed point $x^*\in\BR_\eta(x^*)$, a profile $\varsigma$ following the
public visit-routing convention, and a horizon $T$. List the complete tail visits
contained in $[1,T]$ in chronological order as
\[
V_1,\dots,V_m,
\]
and write
...
\ell_r:=\ell(V_r).
\]
For player $i$, define the cumulative gain term
\begin{equation}
\label{eq:gain-chain-def}
\mathfrak G_{i,T}(\varsigma)
:=
\sum_{r=1}^m g_{i,C_r}^*\,\ell_r,
\end{equation}
the cumulative boundary chain
\begin{equation}
\label{eq:boundary-chain-def}
\mathfrak B_{i,T}(\varsigma)
:=
\sum_{r=1}^m \sum_{b\in B_{C_r}} I_{V_r}(b)\,v_{i,C_r}^{x^*}(b),
\end{equation}
and the total complete-tail length
\[
L_T(\varsigma):=\sum_{r=1}^m \ell_r.
\]

\begin{lemma}[Boundary-chain identity]
\label{lem:boundary-chain}
For every player $i$, every profile $\varsigma$ following the public
visit-routing convention, and every horizon $T$,
\begin{equation}
\label{eq:boundary-chain-identity}
\sum_{V\in\mathcal V_T(\varsigma)}
U_i^{\mathrm{act}}(V;\varsigma)
=
\sum_{V\in\mathcal V_T(\varsigma)}
...
\begin{proof}
This is the definition of $\widetilde U_i(V;\varsigma)$ from
\eqref{eq:visit-aux-payoff}, summed over the chronological visit family.
\end{proof}

\begin{lemma}[Tree telescoping benchmark]
\label{lem:tree-telescoping}
Fix a player $i$, a fixed point $x^*\in\BR_\eta(x^*)$, an initial state $s$, and
a horizon $T$. Then there exist a rooted benchmark
\[
\Gamma_{i,T}(s;x^*)
\]
and finite constants $A_\eta^{\mathrm{tel}},K_\eta^{\mathrm{tel}}<\infty$ such
that for every profile $\varsigma$ following the public visit-routing convention,
\begin{equation}
\label{eq:tree-telescoping}
\left|
\E\bigl[\mathfrak G_{i,T}(\varsigma)-\mathfrak B_{i,T}(\varsigma)\bigr]
-
\Gamma_{i,T}(s;x^*)
\right|
\le
A_\eta^{\mathrm{tel}}\eta\,\E\bigl[L_T(\varsigma)\bigr]
+
K_\eta^{\mathrm{tel}}.
\end{equation}
\end{lemma}
...
\begin{proof}
Write the visits in chronological order. If a visit $V_r$ in node $C_r$ exits
through boundary label $b_r$, then the next visit starts in the unique child
\[
C_{r+1}=C_r[b_r].
\]
The term $v_{i,C_r}^{x^*}(b_r)$ appears with negative sign in the boundary chain
\eqref{eq:boundary-chain-def}, and it is exactly the continuation datum used in
the Bellman problem of the successor node $C_{r+1}$. At a fixed point, the frozen
continuation constraint \eqref{eq:frozen-continuation} identifies that datum with
the child-node gain vector up to $\eta$. Consequently the quantity
\[
g_{i,C_r}^*\,\ell_r
-
\sum_{b\in B_{C_r}}I_{V_r}(b)\,v_{i,C_r}^{x^*}(b)
\]
can be spliced to the next visit's contribution without invoking any gain
near-constancy claim. Iterating this splicing along the unique rooted parent-child
chain cancels all intermediate continuation terms. What remains is a rooted
benchmark depending only on the initial state, the horizon cut, and the fixed
point $x^*$.

The only errors in this reduction come from the $\eta$-slack in
\eqref{eq:frozen-continuation} and from the two endpoint fragments of the chained
Bellman decomposition. The former contributes at most
$A_\eta^{\mathrm{tel}}\eta\,\E[L_T(\varsigma)]$ after summing over the visit
family. The latter is uniformly bounded because the tree has depth at most
$D_\eta$ and every local bias is bounded by $H_\eta$ from
\Cref{prop:bias-span}. Absorbing the endpoint contribution into
$K_\eta^{\mathrm{tel}}$ proves \eqref{eq:tree-telescoping}.
\end{proof}

Fix once and for all a constant
\[
A_\eta\ge 1+A_\eta^{\mathrm{tel}}.
\]
This single constant will absorb both the one-visit $\eta$-slack from
\Cref{lem:visit-bellman} and the tree-level $\eta$-slack from
\Cref{lem:tree-telescoping}.

\begin{proposition}[Compliant total benchmark]
\label{prop:routing-balance}
Fix a player $i$, a fixed point $x^*\in\BR_\eta(x^*)$, and the realized profile
$\sigma_{\eta,\delta,M_*}$. Then there exists a finite constant
$K_\eta^{\mathrm{bal}}$ such that for every horizon $T$ and every initial state
$s$,
\begin{equation}
\label{eq:routing-balance}
\E\left[
\sum_{V\in\mathcal V_T(\sigma_{\eta,\delta,M_*})}
U_i^{\mathrm{act}}(V;\sigma_{\eta,\delta,M_*})
\right]
\ge
\Gamma_{i,T}(s;x^*)
-
(A_\eta\eta+C_\eta\delta)\,
\E\bigl[L_T(\sigma_{\eta,\delta,M_*})\bigr]
-
K_\eta^{\mathrm{bal}}.
\end{equation}
\end{proposition}
...
\begin{proof}
Write $\sigma:=\sigma_{\eta,\delta,M_*}$. Summing
\eqref{eq:visit-aux-realization} visit by visit yields
\[
\E\left[
\sum_{V\in\mathcal V_T(\sigma)}
\widetilde U_i(V;\sigma)
\right]
\ge
\E\bigl[\mathfrak G_{i,T}(\sigma)\bigr]
-
C_\eta\delta\,\E\bigl[L_T(\sigma)\bigr].
\]
Using \Cref{lem:boundary-chain},
\[
\E\left[
\sum_{V\in\mathcal V_T(\sigma)}
U_i^{\mathrm{act}}(V;\sigma)
\right]
=
\E\left[
\sum_{V\in\mathcal V_T(\sigma)}
...
\right]
-
\E\bigl[\mathfrak B_{i,T}(\sigma)\bigr].
\]
...
\right]
\ge
\E\bigl[\mathfrak G_{i,T}(\sigma)-\mathfrak B_{i,T}(\sigma)\bigr]
-
C_\eta\delta\,\E\bigl[L_T(\sigma)\bigr].
\]
Apply \Cref{lem:tree-telescoping} to the compliant profile and absorb
$K_\eta^{\mathrm{tel}}$ into $K_\eta^{\mathrm{bal}}$ to obtain
\eqref{eq:routing-balance}.
\end{proof}

\begin{remark}[Optional semi-Markov viewpoint]
Under the compliant profile, the sequence of visited nodes together with their
sojourn times forms a finite semi-Markov chain on the tree nodes. That viewpoint
remains available, but it is no longer used in the finite-horizon proof. The
benchmark in v19 comes from the chained quantity
$\mathfrak G_{i,T}-\mathfrak B_{i,T}$ and the rooted telescoping lemma, not from
any claim that the node gains are asymptotically constant across the tree.
\end{remark}

Fix a fixed point $x^*\in\BR_\eta(x^*)$ and the realized profile
$\sigma_{\eta,\delta,M_*}$ from \Cref{thm:realization}. For a complete tail
visit $V$ played in node $C$, let $X_{\mathrm{in}}(V)$ denote its entry
phase-state and let $X_{\mathrm{out}}(V)\in\Xi_C^{\mathrm{aug}}$ denote the
terminal augmented state of the visit. Extend each bias $h_{i,C}^*$ to
...
\begin{lemma}[Visitwise Bellman inequality]
\label{lem:visit-bellman}
There exists a constant $C_\eta<\infty$ such that for every complete tail visit
$V$ with announced cap $L(V)\ge M_*$ played in node $C$, every player $i$, and
every unilateral deviation $\tau_i$,
\begin{equation}
\label{eq:visit-bellman-ineq}
\begin{aligned}
\E\left[
\widetilde U_i(V;\tau_i,\sigma_{-i})
\Bigm|\cF_{\mathrm{in}(V)}
\right]
&\le
...
\\
&\quad+
h_{i,C}^*(X_{\mathrm{in}}(V))
-
\E\bigl[h_{i,C}^*(X_{\mathrm{out}}(V))\mid\cF_{\mathrm{in}(V)}\bigr].
\end{aligned}
\end{equation}
\end{lemma}

\begin{proof}
Fix $V$, $C$, and $i$. The Bellman certificate at the fixed point gives, for
...
\nonumber
\end{align}
Inside a complete tail visit, the compliant players are run by the controller
$\mathcal R_{C,L(V)}^{\sigma_C^*}$. By \Cref{ass:controller-input}, at every live
...
\eqref{eq:exact-bellman-fixed-point}; there is no coefficient-perturbation term
inside the visit itself.

Summing the Bellman inequality from the visit entry up to the stopping time
$\ell(V)$ and taking conditional expectations gives
\eqref{eq:visit-bellman-ineq}. The boundary bias term is zero on a strict exit by
the extension convention, and it is the carried-over internal bias when the visit
ends by cap.
\end{proof}

The preceding inequality is purely one-profile: it bounds a deviator visit using
the deviator's own stopping time. We now sum it over the deviator's complete
visit family and then subtract the same boundary chain as in
\Cref{lem:boundary-chain}.

\begin{proposition}[Deviator total benchmark]
\label{prop:deviation-total}
Fix a player $i$, a fixed point $x^*\in\BR_\eta(x^*)$, the realized profile
$\sigma_{\eta,\delta,M_*}$, and a unilateral deviation $\tau_i$. Write
\[
\sigma^{-i}:=(\sigma_{\eta,\delta,M_*})_{-i},
\qquad
\varsigma^\tau:=(\tau_i,\sigma^{-i}).
\]
Then there exists a finite constant $K_\eta^{\mathrm{dev}}$ such that for every
horizon $T$ and every initial state $s$,
\begin{equation}
\label{eq:deviation-total}
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\tau)}
U_i^{\mathrm{act}}(V;\varsigma^\tau)
\right]
\le
\Gamma_{i,T}(s;x^*)
+
...
V_1,\dots,V_m
\]
be the visits in $\mathcal V_T(\varsigma^\tau)$, listed in chronological order,
and write $C_r:=C(V_r)$ and $\ell_r:=\ell(V_r)$. Define
\[
\Delta_r
:=
\widetilde U_i(V_r;\varsigma^\tau)-g_{i,C_r}^*\,\ell_r.
\]
By \Cref{lem:visit-bellman},
\[
\E\bigl[\Delta_r\mid\cF_{\mathrm{in}(V_r)}\bigr]
\le
\eta\,\ell_r
+
h_{i,C_r}^*(X_{\mathrm{in}}(V_r))
-
\E\bigl[h_{i,C_r}^*(X_{\mathrm{out}}(V_r))\mid\cF_{\mathrm{in}(V_r)}\bigr].
\]
Apply \Cref{lem:aggregation} with $a=\eta$, $H=H_\eta$, and $D=D_\eta$. This
gives
\[
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\tau)}
\widetilde U_i(V;\varsigma^\tau)
\right]
\le
...
\right]
-
\E\bigl[\mathfrak B_{i,T}(\varsigma^\tau)\bigr].
\]
Therefore
\[
\E\left[
...
\right]
\le
\E\bigl[\mathfrak G_{i,T}(\varsigma^\tau)-\mathfrak B_{i,T}(\varsigma^\tau)\bigr]
+
\eta\,\E\bigl[L_T(\varsigma^\tau)\bigr]
+
K_{\eta,1}^{\mathrm{dev}}.
\]
Apply \Cref{lem:tree-telescoping}. Since $A_\eta\ge 1+A_\eta^{\mathrm{tel}}$,
absorb the constants into $K_\eta^{\mathrm{dev}}$ and add the harmless
nonnegative term $C_\eta\delta\,\E[L_T(\varsigma^\tau)]$ to obtain
\eqref{eq:deviation-total}.
\end{proof}
...
\smallskip
\noindent\textbf{Step 1: fixed point and gain scale.}
Choose $\eta>0$ so small that
\[
A_\eta\eta\le \frac{\eps}{6}.
\]
...
\Cref{thm:realization,prop:routing-balance,prop:deviation-total}. Choose
$\delta>0$ such that $C_\eta\delta\le \eps/6$. Since the tree is finite, there
is a common cap threshold
\[
...
Let $K_\eta<\infty$ absorb the remainder from \Cref{prop:routing-balance}, the
remainder from \Cref{prop:deviation-total}, the endpoint remainder from
\Cref{lem:aggregation}, the rooted boundary-chain remainder from
\Cref{lem:tree-telescoping}, the finitely many prelude visits before the tail
cap $M_*$ is reached, and the final incomplete visit cut by the horizon.
...
deviation $\tau_i$. Write
\[
\varsigma^\sigma:=\sigma^\eps,
\qquad
\varsigma^\tau:=(\tau_i,\sigma^\eps_{-i}),
\]
and
\[
\mathcal V_T^\sigma:=\mathcal V_T(\varsigma^\sigma),
\qquad
\mathcal V_T^\tau:=\mathcal V_T(\varsigma^\tau).
\]
By the same decomposition used in the proof of
\Cref{thm:realization}\textnormal{(Q3)}, the expected number of stages omitted
from the complete tail visits is uniformly bounded for both profiles. Hence,
after enlarging $K_\eta$ if necessary, both
$\E[L_T(\varsigma^\sigma)]$ and $\E[L_T(\varsigma^\tau)]$ lie between
$T-K_\eta$ and $T$.

For the compliant profile, \Cref{prop:routing-balance} gives
\begin{equation}
\label{eq:compliant-gamma-lower}
\E\left[
\sum_{V\in\mathcal V_T^\sigma}
U_i^{\mathrm{act}}(V;\sigma^\eps)
\right]
\ge
\Gamma_{i,T}(s;x^*)
-
(A_\eta\eta+C_\eta\delta)\,\E\bigl[L_T(\varsigma^\sigma)\bigr]
-
K_\eta.
\end{equation}

For the deviating profile, \Cref{prop:deviation-total} gives
\begin{equation}
\label{eq:deviation-gamma-upper}
\E\left[
\sum_{V\in\mathcal V_T^\tau}
U_i^{\mathrm{act}}(V;\tau_i,\sigma^\eps_{-i})
\right]
\le
\Gamma_{i,T}(s;x^*)
+
(A_\eta\eta+C_\eta\delta)\,\E\bigl[L_T(\varsigma^\tau)\bigr]
+
K_\eta.
\end{equation}

Finally use the horizon decomposition \eqref{eq:realization-actual-tail-bound}
for both profiles. Enlarging $K_\eta$ once if necessary,
\eqref{eq:compliant-gamma-lower} and \eqref{eq:deviation-gamma-upper}
imply
\[
...
\gamma_i^T(s,\sigma^\eps)
\le
2A_\eta\eta + 2C_\eta\delta + \frac{K_\eta}{T}.
\]
...
under condition~\textnormal{(a)} and the chained telescoping benchmark from
\Cref{lem:tree-telescoping}.

The conceptual point of v19 is that the proof no longer attempts to flatten the
whole tree to a single nearly constant gain. That claim is false in general:
local gains may differ by $O(1)$ from parent to child. The correct invariant is
instead the chained gain-minus-boundary quantity
\[
\mathfrak G_{i,T}(\varsigma)-\mathfrak B_{i,T}(\varsigma)
=
\sum_{V\in\mathcal V_T(\varsigma)} g_{i,C(V)}^*\,\ell(V)
-
\sum_{V\in\mathcal V_T(\varsigma)}\sum_{b\in B_{C(V)}}I_V(b)\,v_{i,C(V)}^{x^*}(b).
\]
The continuation term generated when one visit exits is the same datum that
appears at the start of the next visit's Bellman problem, so intermediate
boundary contributions telescope through the rooted tree. Only the initial and
terminal endpoints survive, and those are uniformly bounded by the bias span and
the finite tree depth.

Continuation values therefore remain exactly where they belong: inside the
stopped Bellman equations and inside the cross-visit accounting identity
\[
U_i^{\mathrm{act}} = \widetilde U_i - \text{boundary chain}.
\]
For compliant play, the controller yields a lower bound on the total
$\widetilde U_i$ and hence on the total actual payoff after subtracting the same
boundary chain. For deviating play, the Bellman certificate and the visit
aggregation lemma control the deviator's own sum of $\widetilde U_i$, and
subtracting that same boundary chain again reduces the comparison to the rooted
benchmark $\Gamma_{i,T}(s;x^*)$.

A semi-Markov interpretation of the compliant node process is still available,
but it is no longer the engine of the proof. The finite-horizon comparison closes
already at the level of chained visit accounting:
\[
\text{actual payoff}
=
\text{exit-corrected Bellman total}
-
\text{boundary chain},
\]
together with the fact that the gain-boundary chain telescopes through the tree
without any gain-constancy hypothesis.
\appendix
...
\section{Aggregating visit estimates}
\label{app:aggregation}

\begin{lemma}
\label{lem:aggregation}
Let $V_1,\dots,V_m$ be the complete tail visits contained in $[1,T]$, listed in
chronological order. For each $r$, let $C_r$ be the node of $V_r$,
let $\ell_r:=|V_r|$ be its realized length, and let $\Delta_r$ be a visit
contribution.
Assume there exist constants $a\ge 0$, $H\ge 0$, $D\in\N$, and $K<\infty$ such
that:
\begin{enumerate}[label=\textnormal{(\roman*)},leftmargin=2.6em]
\item $\sum_{r=1}^m \ell_r\le T$;
\item for every $r$ there exist $\cF_{\mathrm{in}(V_r)}$-measurable random
variables $\Psi_r$ and $\widetilde\Psi_r$ with
$|\Psi_r|,|\widetilde\Psi_r|\le H$ a.s. and
\[
\E\bigl[\Delta_r\mid \cF_{\mathrm{in}(V_r)}\bigr]
\le
a\,\ell_r
+
\Psi_r-\E\bigl[\widetilde\Psi_r\mid \cF_{\mathrm{in}(V_r)}\bigr];
\]
\item whenever $V_r$ and $V_{r+1}$ lie in the same node and $V_r$ ends by cap
rather than by a strict exit, one has
$\widetilde\Psi_r=\Psi_{r+1}$ almost surely;
\item the number of indices $r$ for which $C_{r+1}\neq C_r$ is at most $D$;
\item the prelude visits and incomplete terminal fragment outside the complete
tail visits contribute at most $K$ in expectation.
\end{enumerate}
Then
\[
...
\begin{proof}
Partition $V_1,\dots,V_m$ into maximal consecutive runs on which the node is
...
\]
Within each run, the intermediate potentials cancel by assumption~(iii). Each
run therefore leaves at most one initial and one terminal term, each bounded in
absolute value by $H$. Because there are at most $D+1$ runs,
\[
\E\left[\sum_{r=1}^m \Delta_r\right]
\le
a\sum_{r=1}^m \ell_r + 2(D+1)H.
\]
Using assumption~(i) and adding the incomplete-fragment bound from
assumption~(v) yields
\[
\E\left[\sum_{r=1}^m \Delta_r\right]
\le
