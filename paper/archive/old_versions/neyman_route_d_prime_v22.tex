...
Version~22 keeps the v21 stock repair and removes the last v21 bottleneck. The
raw nodewise bias potential
\[
...
The point of v22 is that no separate rooted semi-Markov benchmark is introduced.
Once the frozen public routing memory and the carried stock are included in the
state, the tail dynamics against fixed nondeviators form a finite unilateral-control
MDP for the deviating player. The lifted potential $\Psi$ is a bounded bias on
that routed state space, the compliant routed control attains the Bellman
equalities, and any unilateral deviation satisfies the corresponding upper
inequalities. Standard finite-state multichain comparison therefore gives the
direct regret estimate
\[
...
\subsection{The v22 repair}

Version~22 keeps the v10--v18 frozen-continuation, local-Nash,
...
the v20 stagewise architecture, keeps the v21 stock variable, and changes only
the final global comparison step: the failed rooted-benchmark lemma is deleted
and replaced by a direct routed-control argument.
...
\item Version~22 does not try to identify $\sum_t g_{i,C_t}^*$ with a separate
rooted semi-Markov benchmark. Instead, once the frozen public routing memory and
the carried stock are included in the state, the tail dynamics against fixed
nondeviators form a finite unilateral-control MDP for player $i$.

\item On that routed state space the same Bellman equalities and inequalities are
already global. The stock absorbs the parent/child mismatch, the compliant
routed control attains equality, and any unilateral deviation satisfies the
corresponding upper inequality. Standard finite-state multichain comparison then
yields the direct tail-payoff estimate.
...
stagewise potential drift lemma, the direct routed tail comparison lemma, and
finally \Cref{thm:main}. The appendices record the finite-MDP facts, including
the routed multichain comparison theorem, and the stagewise summation lemma.
...
Version~22 keeps it local and performs the global telescope instead with the
...
\begin{lemma}[Direct routed tail comparison]
\label{lem:direct-tail-comparison}
Fix a player $i$, a fixed point $x^*\in\BR_\eta(x^*)$, the realized profile
$\sigma_{\eta,\delta,M_*}$, an initial state $s$, a horizon $T$, and a
unilateral deviation $\tau_i$. Write
\[
...
\le
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\sigma)}
U_i^{\mathrm{act}}(V;\varsigma^\sigma)
\right]
+
\bigl(A_\eta^{\mathrm{dir}}\eta+C_\eta\delta\bigr)\,T
+
K_\eta^{\mathrm{dir}}.
\end{equation}
\end{lemma}
...
\begin{proof}
After the finite prelude before the frozen tail cap is reached, the public memory
from \Cref{prop:public-routing}\textnormal{(R4)}, the current node, the current
phase-state, and the carried stock value together form a finite routed state
space for player $i$. Once the nondeviators are fixed at
$(\sigma_{\eta,\delta,M_*})_{-i}$, the tail process is therefore a finite
unilateral-control MDP whose stage reward is the actual stage payoff $u_i$.

On a routed tail state $y$, let $C(y)$ be its current node and define
\[
G_i(y):=g_{i,C(y)}^*,
\qquad
H_i(y):=\Psi_i(y).
\]
The point of the stock construction is that the one-step inequalities from
\Cref{lem:visit-bellman} are already the global Bellman inequalities on this
routed MDP. Indeed, if the current routed state lies in node $C$ and the next
stage exits through $b$ into $D=C[b]$, then the stock increment
$v_{i,C}^{x^*}(b)-h_{i,D}^*(X_{t+1})$ converts the parent-node exit credit into
the successor routed-state bias. Thus there is no further tree-level correction
to add. Equality holds on the compliant routed control, and the only one-step
slack for an arbitrary unilateral action is the $+\eta$ term already recorded in
\eqref{eq:stagewise-potential-deviator}. Because \eqref{eq:psi-bound} gives the
uniform bound $\|H_i\|_\infty\le \Psi_\eta^{\max}$,
\Cref{prop:mdp-direct-comparison} applied to this finite routed MDP yields
\[
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\tau)}
u_i^t(\varsigma^\tau)
...
\le
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\sigma)}
u_i^t(\varsigma^\sigma)
\right]
+
A_\eta^{\mathrm{dir}}\eta\,T
+
2\Psi_\eta^{\max}.
\]
Now use
\[
\sum_{t\in\mathcal T_T(\varsigma)}
u_i^t(\varsigma)
=
\sum_{V\in\mathcal V_T(\varsigma)} U_i^{\mathrm{act}}(V;\varsigma)
\]
for both profiles. For the compliant profile we retain the harmless realization
slack $C_\eta\delta\,T$ already inserted in
\eqref{eq:stagewise-potential-compliant}. Absorbing $2\Psi_\eta^{\max}$ into
$K_\eta^{\mathrm{dir}}$ gives \eqref{eq:direct-tail-comparison}.
\end{proof}

\begin{remark}[Why v22 needs no rooted benchmark]
The failed v21 tree-telescoping lemma tried to compare both profiles to an
external rooted semi-Markov quantity. Version~22 does not do that. The
comparison is performed directly in the routed unilateral-control MDP generated
by the frozen public routing convention, and the stock-augmented potential
$\Psi$ already contains the only parent/child repair required to make the
Bellman comparison global.
\end{remark}
...
Let $K_\eta<\infty$ absorb the remainder from
\Cref{lem:direct-tail-comparison}, the finite prelude before the tail cap
$M_*$ is reached, and the final incomplete fragment cut by the horizon in
\Cref{thm:realization}\textnormal{(Q3)} for both the compliant and deviating
profiles. Choose $T_0$ so large that
\[
...
\]
By \Cref{lem:direct-tail-comparison},
\begin{equation}
...
\right]
\le
\bigl(A_\eta\eta+C_\eta\delta\bigr)\,T
+
K_\eta.
\end{equation}
Apply the horizon decomposition \eqref{eq:realization-actual-tail-bound} to both
profiles and enlarge $K_\eta$ once if necessary. Then
\eqref{eq:direct-tail-comparison-proof} implies
\[
...
under condition~\textnormal{(a)} and the direct routed-control comparison from
\Cref{lem:direct-tail-comparison}.

The conceptual point of v22 is that the global comparison is \emph{not}
visitwise, and it does not require a separate rooted semi-Markov benchmark. The
v19 auxiliary functional
\[
\widetilde U_i(V)
=
U_i^{\mathrm{act}}(V)
...
The corrected v22 proof introduces that stock explicitly:
\[
\Psi_{i,t}
=
h_{i,C_t}^*(X_t)+Z_{i,t},
...
Once the frozen public memory and the carried stock are included in the state,
the unilateral problem against fixed nondeviators is already a finite routed MDP
for the deviating player. Version~21 still tried to identify the gain sum with a
separate rooted semi-Markov object. Version~22 deletes that extra layer. The
compliant routed control is compared directly to every unilateral deviation on
that routed state space, and the boundedness of $\Psi$ turns the comparison into
an $O(\eta+\delta+1/T)$ regret estimate after the usual prelude and end-fragment
bookkeeping.

In short, v22 keeps continuation values inside the Bellman drift where they are
legitimate, makes the carried stock explicit, and compares payoffs directly on
the routed unilateral-control problem rather than through a separate tree
benchmark.
\appendix
...
\begin{proposition}[Finite-horizon comparison from the multichain optimality
equations]
\label{prop:mdp-direct-comparison}
Consider a finite controlled Markov chain with bounded stage reward $r$. Let
$\pi$ be a stationary policy. Suppose there are bounded functions $G$ and $H$ on
the state space such that $(G,H,\pi)$ satisfy the multichain average-reward
optimality equations in the sense of \cite[Chapter~8]{Puterman1994}, and suppose
the Bellman inequalities for arbitrary actions hold with an additive slack at
most $\bar\eta\ge 0$. Then for every initial state, every horizon $T$, and every
admissible control $\mu$,
\[
\E^\mu\Bigl[\sum_{t=1}^T r_t\Bigr]
\le
\E^\pi\Bigl[\sum_{t=1}^T r_t\Bigr]
+
\bar\eta\,T
+
2\|H\|_\infty.
...
\begin{proof}
This is the standard finite-horizon overtaking comparison attached to the
multichain average-reward optimality equations; see
\cite[Chapter~8]{Puterman1994}. The additive Bellman slack accumulates linearly
over the horizon, while the bias contributes only the bounded endpoint term
$H(X_1)-H(X_{T+1})$.
\end{proof}
