...
Version~8 keeps the v7 repairs and removes the last restart-bias term in
the bookkeeping/renewal-reward interface. The bookkeeping kernel with restart
through $\rho_C$ is used only to define the stationary rate package
$\bar\mu_C^*=(\mu_C^*,\beta_C^*)$, and in v8 that restart is fixed at the
...
\subsection{The v8 repair}

Version~8 keeps the v7 frozen-continuation, local-Nash, bias-normalization,
augmented-kernel, routing, and renewal-reward repairs, and fixes the remaining
restart-bias bookkeeping term.
...
restart-bias
$\sum_{\xi'}(R_C\beta_C^*)(\xi')h_{i,C}^*(\xi')$, and in v8 it vanishes because
every bookkeeping restart returns to the reference state
$\xi_C^\circ$ where $h_{i,C}^*(\xi_C^\circ)=0$. In the horizon-$T$ accounting,
one sums the actual block durations and uses
...
The central conceptual distinction in v8 is the separation between bookkeeping and
