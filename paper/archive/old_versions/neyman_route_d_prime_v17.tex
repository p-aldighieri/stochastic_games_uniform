...
\title{\textbf{Uniform Equilibrium from an Augmented Metastable Tree\\via Node-Visit Occupation Decomposition}}
...
Version~17 removes the two remaining breaks identified after the v16 review.
Execution never leaves the actual stochastic-game payoff model
\[
u_i(s_t,a_t)\in[0,1].
\]
...
routing automaton stays in a fixed node. The proof no longer compares a deviating
visit to a compliant visit one by one. Those two profiles generate different
visit sequences and different stopping-time laws, so they are treated only after
summing over their own complete visit families. For compliant play, the local
controller gives
\[
\E[U_i^{\mathrm{act}}(V)] = \alpha_{i,C}\,\E[\ell(V)],
\qquad
\E[I_V(b)] = \beta_C(b)\,\E[\ell(V)],
\]
and summing these identities over all compliant visits yields
\[
\E\Bigl[\sum_{V\in\mathcal V_T^\sigma}U_i^{\mathrm{act}}(V)\Bigr]
=
g_{i,\mathrm{root}}^*\,T
+
O_\eta(1)
...
O_\eta(\delta T)
\]
by the fixed-point relation and the occupation balance of the compliant
stationary routing process. For an arbitrary unilateral deviation, the Bellman
certificate and optional stopping are applied visit by visit to the deviator's
own auxiliary functional
\[
\widetilde U_i(V)
=
U_i^{\mathrm{act}}(V)+\sum_{b\in B_C}I_V(b)\,v_{i,C}(b),
\]
which yields the separate upper bound
\[
\E\Bigl[\sum_{V\in\mathcal V_T^\tau}U_i^{\mathrm{act}}(V)\Bigr]
\le
g_{i,\mathrm{root}}^*\,T
+
O_\eta(1)
...
The routing step is therefore performed only after summing over all visits in a
single profile. No argument matches a compliant visit with a deviating visit,
and no child benchmark is weighted by a parent visit length. Because the rooted
tree has finite depth, the total auxiliary/actual discrepancy over the whole
horizon is $O_\eta(1)$. The final regret bound is
\[
...
\subsection{The v17 repair}

Version~17 keeps the v10-v16 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
expectation-bookkeeping, and finite-memory repairs, and it fixes the two
remaining breaks from the Pass~96 review.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The proof no longer compares a deviating visit to a compliant visit on a
single-visit basis. The compliant and deviating profiles have different visit
sequences and, in general, different stopping-time laws for their visits.
Accordingly, the proof now derives a lower bound for the \emph{total compliant}
payoff and a separate upper bound for the \emph{total deviator} payoff.
\item For compliant play, the controller is used only through expectation
identities:
\[
\E[U_i^{\mathrm{act}}(V)] = \alpha_{i,C}\,\E[\ell(V)],
\qquad
\E[I_V(b)] = \beta_C(b)\,\E[\ell(V)].
\]
Summing these identities over all compliant visits in the horizon gives
\[
\E\Bigl[\sum_{V\in\mathcal V_T^\sigma}U_i^{\mathrm{act}}(V)\Bigr]
=
\sum_{V\in\mathcal V_T^\sigma}\alpha_{i,C(V)}\,\E[\ell(V)],
\]
and the fixed-point occupation balance of the compliant stationary routing
process turns this total into
$g_{i,\mathrm{root}}^*T+O_\eta(1)+O_\eta(\delta T)$.
\item For a unilateral deviation, each deviator visit is controlled
independently by the Bellman bound
\[
\E[\widetilde U_i(V)]\le g_{i,C(V)}^*\,\E[\ell(V)] + \text{bias increment},
\]
with the deviator's own visit length on the right-hand side. Summing over the
deviator's complete visit family yields
\[
\E\Bigl[\sum_{V\in\mathcal V_T^\tau}U_i^{\mathrm{act}}(V)\Bigr]
\le
g_{i,\mathrm{root}}^*T+O_\eta(1)+O(\eta T+\delta T).
\]
No compliant visit is used in that estimate.
\item The routing/telescoping step is now performed only after summing over all
visits in a single profile. Each node is weighted by its own total expected
visit duration in that profile. No child term is multiplied by a parent visit
length.
...
contribution is also $O_\eta(1)$. Hence the final regret estimate remains of
order
\[
\eta + C_\eta\delta + \frac{K_\eta}{T}.
\]
...
compliant total balance, the visitwise Bellman bound, the deviator total
benchmark, and finally \Cref{thm:main}. The appendices record the finite-MDP facts and the final visit
...
\subsection{Compliant total balance and deviator Bellman bound}

The proof from this point on keeps the compliant and deviating profiles
separate. A compliant visit and a deviating visit launched from the same public
node need not have the same stopping-time law, so there is no valid per-visit
comparison between them. Instead we prove a lower bound for the total compliant
payoff and an upper bound for the total deviator payoff, each summed over that
profile's own complete visit family.
...
\right].
\]
Applying \eqref{eq:visit-actual-realization} visit by visit and summing yields
\[
\E\left[
...
\ge
\sum_{C\in\cT_\eta}
(\alpha_{i,C}^*-C_\eta\delta)\,L_{C,T}^\sigma.
\]
Under the compliant stationary routing process, the vector
$(L_{C,T}^\sigma)_{C\in\cT_\eta}$ is the actual node-time occupation vector of
the induced visit process. Summing the fixed-point identities
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
|\theta_{i,C}|\le \eta,
\]
against those actual node-time weights and using the controller relation
\[
\E\bigl[I_V(b)\mid\cF_{\mathrm{in}(V)}\bigr]
=
\beta_C^*(b)\,\E\bigl[\ell(V)\mid\cF_{\mathrm{in}(V)}\bigr]
\qquad (b\in B_C)
\]
shows that the weighted compliant average satisfies
\[
\sum_{C\in\cT_\eta}\alpha_{i,C}^*\,L_{C,T}^\sigma
\ge
g_{i,\mathrm{root}}^*\,\E\bigl[L_T(\sigma)\bigr]
-
\eta\,\E\bigl[L_T(\sigma)\bigr]
-
K_\eta^{\mathrm{bal}}.
\]
The crucial point is that the whole compliant visit family is summed at once:
each node is weighted by its own total compliant duration, and no child term is
multiplied by a parent visit length. Combining the last two displays gives
...
Now sum the fixed-point benchmarks along the deviator's own visit sequence. The
routing geometry is still the rooted public tree, so strict node changes still
go only to children. Summing over the whole deviating visit family therefore
yields
\[
\sum_{C\in\cT_\eta} g_{i,C}^*\,L_{C,T}^\tau
\le
...
K_{\eta,2}^{\mathrm{dev}}.
\]
This is the repaired routing step: the child contribution is weighted by the
child durations generated by the deviator's own visit family, never by a parent
visit length.
...
\le
\sum_{V\in\mathcal V_T(\varsigma^\tau)}
U_i^{\mathrm{act}}(V;\varsigma^\tau)+V_\eta.
\]
Absorb $K_{\eta,1}^{\mathrm{dev}}+K_{\eta,2}^{\mathrm{dev}}+V_\eta$ into
$K_\eta^{\mathrm{dev}}$, and add the harmless nonnegative term
$C_\eta\delta\,\E[L_T(\varsigma^\tau)]$ to obtain \eqref{eq:deviation-total}.
\end{proof}
...
Choose $\eta>0$ with $\eta\le \eps/6$. By
...
\gamma_i^T(s,\sigma^\eps)
\le
2\eta + 2C_\eta\delta + \frac{K_\eta}{T}.
\]
...
under condition~\textnormal{(a)} and the repaired single-profile routing
balance.

The conceptual point of v17 is that the proof now keeps the compliant and
deviating profiles separate all the way through the regret comparison. The
realized play is always evaluated with the original stage payoff array
$u_i(s_t,a_t)\in[0,1]$. When a node visit exits, the routing automaton moves
immediately to the successor node and actual play resumes there at the next
stage. There is no cemetery phase and no artificial per-stage continuation
payoff.

Continuation values survive only as stopping-time certificates inside the local
Bellman equations. They appear through the auxiliary exit-corrected visit
functional \eqref{eq:visit-aux-payoff}, which adds at most one continuation term
to a node visit. For compliant play, the controller is used only through the
total-payoff identities
\[
\E\left[\sum_{V\in\mathcal V_T^\sigma}U_i^{\mathrm{act}}(V)\right]
=
\sum_{V\in\mathcal V_T^\sigma}\alpha_{i,C(V)}^*\,\E[\ell(V)]
=
g_{i,\mathrm{root}}^*T+O_\eta(1)+O_\eta(\delta T).
\]
For deviating play, the Bellman certificate and optional stopping control each
deviator visit separately and yield
\[
...
This is exactly where v16 still failed. It tried to subtract a compliant visit
benchmark from a deviating visit benchmark even though the two visits need not
share the same stopping-time law, and it weighted a child benchmark by a parent
visit length in the routing telescoping. Version~17 removes both errors. The
comparison is only between the two \emph{total} horizon-$T$ payoffs, and every
routing sum is taken over the complete visit family of a single profile using
that profile's own node durations. Because the rooted tree has finite depth, the
remaining auxiliary/actual discrepancy and horizon-cutoff losses are only
$O_\eta(1)$.
