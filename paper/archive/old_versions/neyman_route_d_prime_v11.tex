...
Version~11 keeps the v10 repairs and implements the terminal-label fix.
The gain/continuation coordinates remain placed in depth-dependent compact
boxes, and the finitely many prelude blocks before the tail scale $M_*$ remain
peeled off into the $K_\eta/T$ remainder. The essential bookkeeping change is
that terminal labels are no longer treated as special leaves in the horizon
analysis. If a block exits to a terminal label $b$, the continuation coordinate
used in the fixed-point package is the fixed vector $w(b)$, defined as the
per-stage average payoff vector of the terminal continuation profile attached to
$b$. Thus terminal exits enter the frozen-continuation map and the local gain
identity exactly like exits to child nodes: through the same boundary term
$\sum_{b\in B_C}\beta_C(b)v_C(b)$. No separate post-terminal regret term is
needed; the only $O(1/T)$ remainder comes from the frozen prelude and the last
incomplete block or terminal segment. The bookkeeping kernel with restart
through $\rho_C$ is used only to define the stationary rate package
$\bar\mu_C^*=(\mu_C^*,\beta_C^*)$, and in the present version that restart is
fixed at the reference state $\xi_C^\circ$. The executed controller, by
contrast, produces a genuine first-exit renewal block of random duration
$\tau\le L$, where $L$ is only a truncation cap. The correct local identity is
therefore the \emph{renewal-reward relation}
\[
g_{i,C}^*
=
...
\subsection{The v11 repair}

Version~11 keeps the v10 frozen-continuation, local-Nash, bias-normalization,
augmented-kernel, routing, and renewal-reward repairs, and implements the
terminal-continuation fix from the conditional review.
...
stops and play continues according to the fixed terminal continuation profile
attached to $b$, whose per-stage average payoff vector is the continuation
value $w(b)$.
\item Terminal labels are handled by the same fixed-point continuation mechanism
as all other exits. For every terminal exit label $b$ at node $C$, the boundary
continuation coordinate is fixed by
\[
v_{i,C}(b)=w_i(b),
\]
where $w(b)$ is the per-stage average payoff vector of the attached terminal
continuation profile. Thus terminal exits contribute to the local gain through
the same boundary term $\sum_{b\in B_C}\beta_C(b)v_{i,C}(b)$ as exits to child
nodes. No separate terminal-regret term is needed; only the usual
$K_\eta/T$ boundary remainder remains.
\end{enumerate}
...
label, the tree input also specifies a fixed terminal continuation profile to be
used for all remaining stages after that exit; its per-stage average payoff
vector is denoted by $w(b)\in[0,1]^N$.
...
terminal label, then $v_C^x(b)=w(b)$ is the per-stage average payoff vector of
the fixed terminal continuation profile attached to that label. Terminal exits
are handled in the local Bellman equations exactly like exits to child nodes:
the continuation value is simply frozen at the exogenous vector $w(b)$. There
is therefore no separate terminal accounting outside the fixed-point system.
Throughout this section $x$ is considered fixed.
...
If the block exits at a terminal label $b$, that second term is the fixed
continuation value $w(b)$, namely the per-stage average payoff vector of the
terminal continuation profile attached to $b$. Terminal exits are treated in
the augmented bookkeeping exactly like any other boundary exit; after such an
exit the public routing rule simply continues with the fixed terminal profile
and launches no further blocks.
...
$\partial_b$ and $C[b]$ is a terminal label, the block schedule stops, the
exiting augmented block reward records the continuation value $w(b)$, and play
continues for all remaining stages according to the fixed terminal continuation
profile attached to $b$. Here $w(b)$ is the per-stage average payoff vector of
that fixed terminal profile. Terminal labels therefore behave like ordinary exit
labels with fixed continuation data, except that no further block is scheduled
once such a label is reached. If no cemetery state is visited, the next block
starts immediately in the same node.
...
label, the exiting block records the continuation value $w(b)$, where $w(b)$ is
the per-stage average payoff vector of the attached terminal profile, control
passes to that fixed terminal continuation profile for all remaining stages, and
no further block is scheduled. If no label is emitted, the controller remains
in the same node. The bound by $D_\eta$ follows from finiteness of the tree and
the fact that every cemetery-routing move to a child strictly decreases the
remaining depth. The memory requirements are finite by
\Cref{lem:frozen-memory}, and all relevant quantities are publicly observable.
\end{proof}
...
node of block $B$. The constant $K_\eta$ absorbs the frozen prelude and the
initial and final incomplete fragments, including the case where the final
fragment lies in the fixed terminal continuation segment after block scheduling
stops.
\end{enumerate}
\end{theorem}
...
For (Q3), decompose the horizon into the complete blocks contained in $[1,T]$
together with the initial and final incomplete fragments. The final fragment may
occur either inside a launched block or inside the fixed terminal continuation
segment that begins after a terminal exit. Among the complete blocks, only
finitely many belong to the frozen prelude and have cap $<M_*$. Their combined
contribution to the realized payoff and to the weighted-gain sum is bounded by
a finite constant depending only on the frozen prelude, the tail cap $M_*$, and
the finite tree object; absorb that constant into $K_\eta$. Summing
...
If the final routed block exits to a terminal label $b$, then the continuation
value carried by that block is $w(b)$, the per-stage average payoff vector of
the fixed terminal profile that governs all remaining stages. This is exactly
the continuation value used in the fixed-point construction, so the terminal
continuation is already internalized in the blockwise Bellman certificate and in
the gain $g_{i,C}^*$. No separate terminal correction term is needed.
...
The terminal bias term is zero on a cemetery exit by the extension convention above,
...
fragments, including the case where the final fragment lies in the fixed
terminal continuation segment after block scheduling stops, contribute at most a
finite constant depending only on the frozen prelude, the tail cap, and the
finite tree object. Absorb all of these terms into a constant $K_\eta<\infty$.
...
complete block differs from its ordinary stage-payoff sum only if it exits to a
cemetery state, in which case one continuation term in $[0,D_\eta]$ is added.
Under either the deviating or the compliant profile, the path can contain at
most $D_\eta$ such exits. Hence the total difference between augmented regret
and ordinary regret over all complete blocks in $[1,T]$ is at most
$2D_\eta^2$, already absorbed into $K_\eta$. If the last routed block exits to a
terminal label $b$, then the continuation value carried by that block is
$w(b)$, the per-stage average payoff vector of the fixed terminal profile that
governs all remaining stages. This is exactly the continuation value used in the
fixed-point Bellman system, so no separate terminal-regret term appears. The
incomplete fragments are already absorbed into $K_\eta$. We obtain
\[
...
The central conceptual distinction in v11 is the separation between bookkeeping
and execution together with the rule that terminal labels are simply exit labels
with fixed continuation values. The restart kernel $\rho_C$ belongs only to the
convex bookkeeping package; it is not an executed transition after a cemetery
hit. Execution takes place on the genuine absorbing augmented kernel, blocks
terminate at the first cemetery state, and the relevant local identity is
renewal-reward: the bookkeeping rates determine $g_{i,C}^*$, while the executed
controller contributes that gain through total augmented reward divided by
actual first-exit time. When a block hits a terminal label, the exiting block
carries the continuation value $w(b)$, where $w(b)$ is the per-stage average
payoff vector of the attached terminal profile, and no separate terminal-regret
correction is needed. With those distinctions in place, the fixed-point domain
is convex, the Bellman target matches the realized first-exit block, and the
routing semantics are well-defined.
...
\item the incomplete fragments outside the complete blocks, including the case
where the final fragment lies in the fixed terminal continuation segment after
block scheduling stops, contribute at most $K$ in expectation.
\end{enumerate}
Then
\[
