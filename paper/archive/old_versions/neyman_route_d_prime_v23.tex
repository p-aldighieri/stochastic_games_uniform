...
Version~23 keeps the stock repair from v21 and the stagewise $\Psi$-telescope
from v22, but it removes the last v22 defect. The telescope itself was correct;
the failure was the subsequent gain-level comparison, which tried to compare
\(\sum_t g_{i,C_t}^*\) along the deviator's routed node sequence with the
compliant benchmark along a different routed node sequence. That comparison is
not legitimate and is not used in v23.

The repair is to use the same stock-augmented potential on both profiles. Under
the compliant profile, the realization identities give the exact nodewise
\(\alpha\)-benchmark. Under a unilateral deviation, the Bellman inequalities
give the \(g\)-benchmark. The nodewise identity
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\]
shows that the benchmark gap is exactly the continuation contribution already
carried by the stock $Z$, hence already contained in
$\Psi_{i,t}=h_{i,C_t}^*(X_t)+Z_{i,t}$. No separate routed gain comparison is
required.

As a result, both the compliant payoff and the deviating payoff are written by
the same bounded-$\Psi$ telescope, and the regret is controlled directly by the
endpoint term:
\[
...
\]
Here the $K_\eta/T$ term comes from the uniform bound on $\Psi$ together with the
usual finite prelude and final-fragment bookkeeping. Choosing the parameters in
that order proves the theorem.
\end{abstract}
...
\subsection{The v23 repair}

Version~23 keeps the v10--v18 frozen-continuation, local-Nash,
...
the v20 stagewise architecture, keeps the v21 stock variable, keeps the v22
$\Psi$-telescope, and changes only the final global comparison step: the routed
gain comparison is deleted and replaced by a direct bounded-$\Psi$ regret
argument.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The structural defect remains the same. The visit functional
\[
...
\item Summing over the horizon yields, for each routed profile separately,
\[
\E\Bigl[\sum_{t=1}^T u_i(s_t,a_t)\Bigr]
=
\E\Bigl[\sum_{t=1}^T \textnormal{benchmark}_{i,t}\Bigr]
+
\E[\Psi_{i,1}-\Psi_{i,T+1}]
+
O\!\left((\eta+\delta)T\right),
\]
where the benchmark is the exact nodewise $\alpha$-rate on the compliant profile
and the nodewise Bellman gain $g$ on the deviating profile. The endpoint term is
harmless because $\Psi$ is uniformly bounded. In the present routed tree one
even has a pathwise bound, since there are at most $D_\eta$ strict child moves.

\item The v22 defect occurred after this point. One cannot compare
\(\sum_t g_{i,C_t}^*\) along the deviator's routed path to the compliant
benchmark along a different routed path. The two profiles need not traverse the
same node sequence, so that routed gain comparison is not legitimate.

\item Version~23 therefore never performs a cross-profile gain comparison. Under
compliant play the realization theorem supplies the exact $\alpha$-benchmark;
under deviation the Bellman inequalities supply the $g$-benchmark. The nodewise
gain identity
\[
g_{i,C}^*
=
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b)
\]
shows that the gap between these benchmarks is exactly the continuation term
already carried by the stock $Z$.

\item Hence the regret is obtained by subtracting the two bounded-$\Psi$
telescopes. The only global cross-profile term is the bounded endpoint term of
$\Psi$, not a routed comparison of gain sums. This yields the direct regret bound
\(O(\eta+\delta+1/T)\).
\end{enumerate}
...
the direct $\Psi$ comparison proposition, and the stagewise summation lemma.
...
Version~23 keeps it local and performs the global telescope instead with the
...
\begin{lemma}[Direct routed tail comparison via \texorpdfstring{$\Psi$}{Psi}]
...
$\varsigma^\sigma$ and the deviation profile $\varsigma^\tau$. The inputs to that
proposition are exactly the two one-profile $\Psi$-telescopes supplied by
\Cref{lem:visit-bellman}\textnormal{(a)--(b)}, together with the node gain
identity \eqref{eq:node-gain-identity} and the stock update
\eqref{eq:psi-stock-update}, which identify the benchmark gap
$g_{i,C}^*-\alpha_{i,C}^*$ with the continuation stock already carried by
$\Psi$.

Therefore, after enlarging $A_\eta^{\mathrm{dir}}$ if necessary to dominate
$A_\eta^{\mathrm{pot}}$, one obtains
\[
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\tau)}u_i^t(\varsigma^\tau)
\right]
\le
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\sigma)}u_i^t(\varsigma^\sigma)
\right]
+
\bigl(A_\eta^{\mathrm{dir}}\eta+C_\eta\delta\bigr)\,T
+
...
Finally use
\[
\sum_{t\in\mathcal T_T(\varsigma)}u_i^t(\varsigma)
=
\sum_{V\in\mathcal V_T(\varsigma)} U_i^{\mathrm{act}}(V;\varsigma)
\]
for both profiles and absorb $2\Psi_\eta^{\max}$ into
$K_\eta^{\mathrm{dir}}$. This gives \eqref{eq:direct-tail-comparison}.
\end{proof}

\begin{remark}[Why v23 needs no routed gain comparison]
The failed v22 tail step still hid a comparison of
\(\sum_t g_{i,C_t}^*\) across different routed node sequences. Version~23 does
not do that. The compliant and deviating payoffs are each written by the same
bounded-$\Psi$ telescope, and the gap $g_{i,C}^*-\alpha_{i,C}^*$ is absorbed by
the carried stock $Z$. Thus the only surviving global term is the bounded
endpoint of $\Psi$.
\end{remark}
...
under condition~\textnormal{(a)} and the direct $\Psi$-comparison from
\Cref{lem:direct-tail-comparison}.

The conceptual point of v23 is not merely that the stock $Z$ repairs the
parent/child boundary mismatch. That part was already correct in v22. The new
point is that once the stock-augmented potential
\[
\Psi_{i,t}
=
h_{i,C_t}^*(X_t)+Z_{i,t},
...
v_{i,C_t}^{x^*}(b)-h_{i,C_t[b]}^*(X_{t+1})
\]
at a strict exit through $b$ is available, one should use it directly in the
equilibrium proof and stop before any routed gain comparison. The v22 comparison
lemma still tried to compare \(\sum_t g_{i,C_t}^*\) along the deviator's routed
node sequence with the compliant benchmark along a different routed node
sequence. That step is deleted in v23.

The compliant profile is handled by the realization identities, which give the
exact nodewise $\alpha$-benchmark. The deviating profile is handled by the
Bellman upper bound, which gives the nodewise $g$-benchmark. The node gain
identity
\[
g_{i,C}^*
=
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b)
\]
shows that the benchmark gap is precisely the continuation contribution already
carried by the stock $Z$. Hence the only global term that survives after
subtraction is the bounded endpoint term of $\Psi$.

So the final regret estimate is a direct bounded-potential argument. One writes
both profiles by the same $\Psi$-telescope, uses the realization theorem on the
compliant side and the Bellman inequality on the deviating side, absorbs the
continuation gap into $Z$, and then pays only $O(\eta+\delta+1/T)$. In short,
v23 keeps continuation values inside the Bellman drift, keeps the explicit stock,
and derives equilibrium directly from $\Psi$-boundedness rather than from a
separate routed gain comparison theorem.
\appendix



\section{Finite average-reward MDP facts and the direct \texorpdfstring{$\Psi$}{Psi} comparison}
\label{app:mdp}
...
\begin{proposition}[Direct comparison from the common bounded potential
\texorpdfstring{$\Psi$}{Psi}]
\label{prop:mdp-direct-comparison}
Fix a player $i$, a fixed point $x^*\in\BR_\eta(x^*)$, the compliant routed
profile $\varsigma^\sigma:=\sigma_{\eta,\delta,M_*}$, and a unilateral deviation
$\varsigma^\tau:=(\tau_i,(\sigma_{\eta,\delta,M_*})_{-i})$. Assume the one-profile
$\Psi$-telescopes of \Cref{lem:visit-bellman}\textnormal{(a)--(b)}, the compliant
realization identity \eqref{eq:visit-actual-realization-exact}, the node gain
identity \eqref{eq:node-gain-identity}, and the stock update
\eqref{eq:psi-stock-update}. Then for every horizon $T$,
\[
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\tau)}u_i^t(\varsigma^\tau)
\right]
...
\right]
\le
A_\eta^{\mathrm{pot}}\eta\,T
+
C_\eta\delta\,T
+
2\Psi_\eta^{\max}.
\]
Equivalently, once the common stock-augmented potential is in place, the final
regret comparison uses only the bounded endpoint term of $\Psi$; no separate
comparison of routed gain sums is required.
\end{proposition}

\begin{proof}
Write the deviating one-profile telescope from
\Cref{lem:visit-bellman}\textnormal{(b)} in the form
\[
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\tau)}u_i^t(\varsigma^\tau)
\right]
...
2\Psi_\eta^{\max}.
\]
For the compliant profile, \Cref{thm:realization}\textnormal{(Q2)} gives the exact
$\alpha$-benchmark
\[
\E\left[
\sum_{t\in\mathcal T_T(\varsigma^\sigma)}u_i^t(\varsigma^\sigma)
...
=
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\sigma)}\alpha_{i,C(V)}^*\,\ell(V)
\right].
\]
Now \eqref{eq:node-gain-identity} rewrites each nodewise gain as
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b).
\]
The continuation term on the right is exactly the quantity carried by the stock:
when stage $t$ exits node $C_t$ through $b$, the increment
\eqref{eq:psi-stock-update} adds
$v_{i,C_t}^{x^*}(b)-h_{i,C_t[b]}^*(X_{t+1})$. Hence the benchmark gap
$g_{i,C}^*-\alpha_{i,C}^*$ is not a second routed comparison term. It is already
contained in the same endpoint contribution of $\Psi$ that appears in the two
one-profile telescopes. Subtracting the compliant telescope from the deviating
one therefore leaves only the endpoint term, together with the explicit
$A_\eta^{\mathrm{pot}}\eta\,T$ and $C_\eta\delta\,T$ errors. Since
$|\Psi_{i,t}|\le \Psi_\eta^{\max}$, the remaining endpoint contribution is at
most $2\Psi_\eta^{\max}$.
\end{proof}
