...
Version~24 keeps the stock repair from v21 and the stagewise $\Psi$-telescope
from v22, but it replaces the last defective v23 step. The problem was that the
identity
\[
g_{i,C}^*
=
...
\]
is a statement about the compliant node data. It may be used to identify the
compliant benchmark, but it cannot be pushed along the deviator-routed node
sequence. Proposition~\ref{prop:mdp-direct-comparison} is therefore rewritten as
a genuinely two-sided argument with a single common benchmark, namely the root
value $g_{i,\mathrm{rt}}^*$.

On the compliant side, the realization identities and the rooted semi-Markov
renewal relation imply
\[
...
\]
On the deviating side, the Bellman certificate and the bounded stock-augmented
potential \(\Psi\) give a supermartingale upper bound
\[
...
\]
Thus v24 keeps the $Z/\Psi$ machinery intact but compares both profiles only
through the common root benchmark. No routed identity of the form
\(g-\alpha=\Delta Z\) is used for the deviator.
\end{abstract}
...
\subsection{The v24 repair}

Version~24 keeps the v10--v18 frozen-continuation, local-Nash,
...
$\Psi$-telescope, and changes only the final comparison step: the two profiles
are now compared through the common root benchmark $g_{i,\mathrm{rt}}^*$.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The structural defect is unchanged. The visit functional
\[
...
\item The Bellman certificate still gives the one-step drift formula
\[
...
\item The remaining v23 defect was subtler. The identity
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b)
\]
belongs to the compliant node data. It identifies the compliant renewal
benchmark inside node $C$, but it is not a deviator-routed identity and cannot
be used to compare different routed node sequences.

\item On the compliant side, one uses the realization identities together with
the fixed-point relation
\(v_{i,C}^{x^*}(b)=g_{i,C[b]}^*+O(\eta)\).
This yields an approximate rooted renewal recursion
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
O(\eta),
\]
and therefore the compliant average payoff equals the root value
$g_{i,\mathrm{rt}}^*$ up to $O(\eta+\delta+1/T)$.

\item On the deviating side, one never rewrites $g-\alpha$ along the deviator
path. The Bellman inequality and the bounded potential $\Psi$ give a
supermartingale upper bound showing that no unilateral deviation can earn more
than the same root value $g_{i,\mathrm{rt}}^*$, up to $O(\eta+1/T)$.

\item Hence the regret is obtained by comparing each profile separately to the
same root benchmark. The final estimate is
\[
\gamma_i^T\bigl(s,(\tau_i,\sigma_{-i})\bigr)-\gamma_i^T(s,\sigma)
\le
A_\eta\eta + C_\eta\delta + \frac{K_\eta}{T}.
\]
No routed identity of the form $g-\alpha=\Delta Z$ is used for the deviator.
\end{enumerate}
...
stagewise potential drift lemma, the direct comparison lemma via the common root
benchmark, and finally \Cref{thm:main}. The appendices record the finite-MDP
facts, including the root-value comparison proposition, and the stagewise
summation lemma.
...
Version~24 keeps it local and performs the global telescope instead with the
...
\begin{lemma}[Direct comparison via the common root benchmark]
...
compares the two profiles only through the common root benchmark
$g_{i,\mathrm{rt}}^*$ and yields
\[
\gamma_i^T\bigl(s,\varsigma^\tau\bigr)
-
\gamma_i^T\bigl(s,\varsigma^\sigma\bigr)
...
\begin{remark}[Why v24 uses the root value]
The defective v23 step still tried to interpret
\(g_{i,C_t}^*-\alpha_{i,C_t}^*\) along the deviator-routed node sequence.
Version~24 does not do that. The identity
\(g_{i,C}^*=\alpha_{i,C}^*+\sum_b\beta_C^*(b)v_{i,C}^{x^*}(b)\) is used only on the
compliant node data to identify the rooted renewal value. The deviator is
controlled separately by the Bellman-$\Psi$ supermartingale, and both profiles
are compared only through $g_{i,\mathrm{rt}}^*$.
\end{remark}
...
under condition~\textnormal{(a)} and the common-root comparison from
\Cref{lem:direct-tail-comparison}.

The conceptual point of v24 is not merely that the stock $Z$ repairs the
parent/child boundary mismatch. That part was already correct in earlier
versions. The critical repair is logical: the identity
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b)
\]
belongs to the compliant node data and cannot be transported to the
deviator-routed node sequence. The compliant profile is therefore treated by a
rooted semi-Markov renewal argument, while the deviator is treated by the
Bellman supermartingale built from the bounded potential
\[
\Psi_{i,t}
=
h_{i,C_t}^*(X_t)+Z_{i,t},
...
The common benchmark is the root value $g_{i,\mathrm{rt}}^*$. Compliant play
achieves $g_{i,\mathrm{rt}}^*$ up to $O(\eta+\delta+1/T)$. A unilateral deviation
cannot exceed $g_{i,\mathrm{rt}}^*$ up to $O(\eta+1/T)$. Once those two one-sided
estimates are separated correctly, the equilibrium proof closes without any
comparison of $g$-sums or $g-\alpha$ identities across different routed paths.
\appendix



\section{Finite average-reward MDP facts and the common root-value comparison}
\label{app:mdp}
...
\begin{proposition}[Two-sided comparison through the common root benchmark]
...
common root benchmark $g_{i,\mathrm{rt}}^*$; no identity of the form
$g-\alpha=\Delta Z$ is used along the deviator-routed path.
\end{proposition}

\begin{proof}
Because condition~\textnormal{(a)} makes every local deviation MDP communicating,
\Cref{prop:bias-unique} identifies $g_{i,C}^*$ as the state-independent average
reward value of the node problem in every node $C$. In particular the root value
$g_{i,\mathrm{rt}}^*$ does not depend on the initial phase-state selected by the
initial stochastic-game state $s$.

At a fixed point, \eqref{eq:frozen-continuation} and
\eqref{eq:node-gain-identity} give, for every ordinary node $C$,
\begin{equation}
...
one has $g_{i,T}^*=w_i(T)=\alpha_{i,T}^*$, so the same recursion is exact.

For a node $C$, let $\Gamma_{i,C}^{T,\sigma}$ denote the expected $T$-stage
average payoff of the continuation play obtained by starting the public routing
automaton in $C$ and then following the compliant profile, and let
$\Gamma_{i,C}^{T,\tau}$ denote the corresponding supremum over unilateral
deviations of player $i$. We prove by induction on the remaining depth
$d_\eta(C)$ that, uniformly over entry phase-states,
\begin{align}
\Gamma_{i,C}^{T,\sigma}
&\ge
...
for all strict children of $C$. Under the compliant profile, each complete tail
...
\]
If that visit exits through $b$, the continuation game starts in the child
...
\eqref{eq:root-benchmark-recursion} yields the compliant bound for $C$.
...
remaining play in that child. Thus the $\Psi$-adjusted payoff process is a
supermartingale dominated by $g_{i,C}^*$, up to the explicit
$A_\eta^{\mathrm{pot}}\eta$ drift term and the bounded endpoint term of $\Psi$.
Since every routed play has at most $D_\eta-1$ strict child moves, all recursive
continuation and endpoint contributions are absorbed into one constant
$K_\eta^{\mathrm{root}}$. This proves the deviator bound for $C$.
