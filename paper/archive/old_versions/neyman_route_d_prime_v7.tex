...
Version~7 keeps the v6 repairs and fixes the remaining bookkeeping/first-exit
mismatch. The bookkeeping kernel with restart through $\rho_C$ is used only to
define the stationary rate package
$\bar\mu_C^*=(\mu_C^*,\beta_C^*)$. The executed controller, by contrast, produces
a genuine first-exit renewal block of random duration $\tau\le L$, where $L$ is
...
\subsection{The v7 repair}

Version~7 keeps the earlier frozen-continuation, local-Nash, bias-normalization,
augmented-kernel, and routing repairs, and fixes the remaining bookkeeping versus
first-exit mismatch.
...
package and is never an executed transition after a cemetery hit.
...
\frac{\E[\widetilde U_{i,C,L}]}{\E[\tau_{C,L}]},
\]
not $\E[\widetilde U_{i,C,L}/L]$. In the horizon-$T$ accounting, one sums the
actual block durations and uses
...
controller's total augmented block reward is
\[
\widetilde U_{i,C,L}
:=
...
package $\bar\mu_C^*=(\mu_C^*,\beta_C^*)$ yields
\[
g_{i,C}^*
=
...
Define the depth of the finite universal tree by
\[
D_\eta
:=
\max\{\text{number of strict child moves on a root-to-leaf path of }\cT_\eta\}.
\]
Because $\cT_\eta$ is finite, one has $D_\eta<\infty$.
...
the child node $C[b]$. If no cemetery state is visited, the next block starts
immediately in the same node.
...
is emitted, the controller remains in the same node. The bound by $D_\eta$ follows
from finiteness of the tree and the fact that cemetery routing moves only to
strict children. The memory requirements are finite by \Cref{lem:frozen-memory},
and all relevant quantities are publicly observable.
\end{proof}
...
Item~(Q2) is the renewal-reward estimate from \Cref{thm:local-controller}, again
conditioned on the entry state and with constants enlarged uniformly over the
finite tree.
...
Summing \eqref{eq:block-payoff-realization} over the complete blocks and using
\[
...
which case the augmented payoff contains one continuation term lying in $[0,1]$.
By \Cref{prop:public-routing}, the play can contain at most $D_\eta$ such exits.
Hence the total augmented/ordinary discrepancy over all complete blocks inside
$[1,T]$ is at most $D_\eta$.
...
\nonumber
\end{align}
Inside a complete tail block, the realized compliant controller is
$\delta$-close to the fixed-point controller in the sense of
\eqref{eq:block-occupation-realization}. Since the coefficient arrays of the
deviation MDP are Lipschitz in the compliant kernel,
...
\eqref{eq:exact-bellman-fixed-point} by at most $C_\eta\delta$ per live stage.
...
Summing the perturbed Bellman inequality from the block entry up to time $\tau(B)$
and taking conditional expectations gives
\begin{align*}
\E\left[\widetilde U_i(B;\tau_i,\sigma_{-i})\Bigm|\cF_{\mathrm{in}(B)}\right]
\le{}&
\bigl(g_{i,C}^*+\eta+C_\eta\delta\bigr)\,
...
\E\bigl[h_{i,C}^*(X_{\mathrm{out}}(B))\mid\cF_{\mathrm{in}(B)}\bigr].
\end{align*}
This is exactly \eqref{eq:block-bellman-ineq}. The terminal term is zero on a
cemetery exit by the extension convention above, and it is the carried-over bias of
the terminal internal state when the block ends at its cap.
\end{proof}
...
\E\left[
\widetilde U_i(B;\tau_i,\sigma_{-i})-\widetilde U_i(B;\sigma_{\eta,\delta,M_*})
\Bigm|\cF_{\mathrm{in}(B)}
\right]
\le
(\eta+C_\eta\delta)\,\E\bigl[|B|\mid\cF_{\mathrm{in}(B)}\bigr]
...
Let $D_\eta$ be the depth of the finite universal tree from
\Cref{prop:public-routing}. By \Cref{lem:aggregation}, the telescoping remainder
over complete blocks is bounded by $2(D_\eta+1)H_\eta$, and the augmented/ordinary
payoff discrepancy over complete blocks is bounded by $2D_\eta$. The incomplete
initial and terminal fragments contribute at most a finite constant depending only
on the frozen prelude, the tail cap, and the finite tree object. Absorb all of
these terms into a constant $K_\eta<\infty$. Choose $T_0$ so large that
\[
\frac{K_\eta}{T_0}\le \frac{\eps}{3}.
\]

Now fix $T\ge T_0$, an initial state $s$, a player $i$, and a unilateral deviation
$\tau_i$. Let $B_1,\dots,B_m$ be the complete blocks contained in $[1,T]$, listed
in chronological order. Apply \Cref{cor:block-regret} block by block and sum.
Within each maximal run of consecutive blocks that stay in the same node, the bias
terms telescope because the terminal internal state of one capped block is exactly
the entry state of the next block in that same node. Every time the node changes,
the play moves to a strict child, and this can happen at most $D_\eta$ times.
Therefore \Cref{lem:aggregation} gives
\[
...
in which case one continuation term in $[0,1]$ is added. Under either the
...
all complete blocks in $[1,T]$ is at most $2D_\eta$, already absorbed into
...
The central conceptual distinction in v7 is the separation between bookkeeping and
