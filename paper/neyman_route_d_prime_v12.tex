...
Version~12 implements the structural terminal fix. Every former terminal boundary
label $b$ is replaced by a \emph{degenerate terminal node} $T_b$, a leaf of the
universal tree with a single internal absorbing state, no exits, constant
per-stage payoff vector $w(b)$, trivial bookkeeping kernel, and exact Bellman
certificate
\[
g_{T_b}=w(b),
\qquad
...
Thus terminal behavior becomes regenerative inside the same block system as every
other node. The proof never leaves the block decomposition, no post-terminal
segment has to be estimated separately, and the spurious $\Theta(T)$ horizon gap
disappears.
...
\subsection{The v12 redesign}

Version~12 keeps the v10-v11 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, and renewal-reward repairs, and
replaces terminal labels by degenerate terminal nodes.
...
most one boundary label and then moves to the corresponding successor node
$C[b]$, which may be an ordinary child or a degenerate terminal child. Blocks
never regenerate through cemetery states.
...
\item The block schedule never stops before horizon $T$. After an exit into a
degenerate terminal node, subsequent blocks are simply the trivial capped blocks
of that node. Hence terminal behavior is covered by the same routing,
realization, telescoping, and regret estimates as all other nodes, and no
special post-terminal accounting is needed.
\end{enumerate}
...
If $C=T$ is degenerate terminal, then $B_C=\varnothing$, so the second term is
always absent and
\[
\widetilde U_i(B)=\sum_{t\in B^{\mathrm{live}}}u_i^t = w_i(T)\,|B|
\]
exactly. Thus the same notation covers ordinary and degenerate nodes uniformly.
...
\item If a block terminates at $\partial_b$, the next block starts immediately in
the successor node $D=C[b]$. This successor may be an ordinary child or a
degenerate terminal child $T_b$. If $D=T_b$, then every subsequent block is
played in $T_b$, no boundary label is ever emitted again, and each such block
simply stays at the unique internal state of $T_b$ for its full cap while
earning $w(T_b)=w(b)$ per stage. If no cemetery state is visited, the next block
starts immediately in the same node.
\item Because routing on cemetery hits always moves to a strict child and
degenerate terminal nodes have no exits, every play path contains at most
$D_\eta$ cemetery exits.
...
the next block starts in the child node $D=C[b]$. If $D$ is degenerate terminal,
then its controller is the trivial one-state controller from the remark after
\Cref{ass:controller-input}, so all subsequent blocks remain there. If no label
is emitted, the controller remains in the same node. The bound by $D_\eta$
follows from finiteness of the tree and the fact that every cemetery-routing move
to a child strictly decreases the remaining depth. The memory requirements are
...
initial and final incomplete fragments.
\end{enumerate}
\end{theorem}
...
together with the initial and final incomplete fragments. Among the complete
blocks, only finitely many belong to the frozen prelude and have cap $<M_*$.
Their combined contribution to the realized payoff and to the weighted-gain sum
is bounded by a finite constant depending only on the frozen prelude, the tail
cap $M_*$, and the finite tree object; absorb that constant into $K_\eta$.
Summing \eqref{eq:block-payoff-realization} over the remaining tail blocks and
using
\[
...
such exits. Blocks played after the route has entered a degenerate terminal node
have $B_C=\varnothing$ and therefore carry no continuation bonus at all. Hence
the total augmented/ordinary discrepancy over all complete blocks inside $[1,T]$
is at most $D_\eta^2$.
...
$2D_\eta^2$, already absorbed into $K_\eta$. Blocks played after entry into a
degenerate terminal node have no cemetery exit and therefore create no extra
discrepancy. The incomplete fragments are already absorbed into $K_\eta$. We obtain
\[
...
The central conceptual distinction in v12 is the separation between
bookkeeping and execution together with the structural replacement of terminal
labels by degenerate terminal nodes. The restart kernel $\rho_C$ belongs only to
the convex bookkeeping package; it is not an executed transition after a
cemetery hit. Execution takes place on the genuine absorbing augmented kernel,
blocks terminate at the first cemetery state, and the relevant local identity is
...
actual first-exit time. When a block hits a boundary label leading to a
degenerate terminal node $T_b$, the next block is simply played in $T_b$, whose
gain is exactly $w(b)$ and whose bias is zero. The block schedule therefore
continues through terminal behavior instead of stopping before the horizon
analysis is finished. With those distinctions in place, the fixed-point domain
