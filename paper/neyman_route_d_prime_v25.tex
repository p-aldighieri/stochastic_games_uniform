...
\title{\textbf{Uniform Equilibrium from an Augmented Metastable Tree\\via an Induced Single-Player MDP Benchmark}}
...
Version~25 changes the interpretation of the final scalar benchmark. When player
$i$ deviates from the realized profile $\sigma_{\eta,\delta,M_*}$, the other
players keep playing $(\sigma_{\eta,\delta,M_*})_{-i}$. The deviator therefore
faces a finite one-player MDP on the routed public-memory state space. The
nodewise Bellman certificates $(g_{i,C}^*,h_{i,C}^*)$ are an approximate
solution of the average-reward Bellman equation for that induced MDP, and the
stock-augmented potential $\Psi$ is a supermartingale certificate for every
unilateral policy. The compliant policy is, in the same induced MDP, the
approximately optimal stationary policy selected at the fixed point.

Accordingly, the scalar $g_{i,\mathrm{rt}}^*$ is not used as a benchmark by
fiat. It is the root value of the induced one-player MDP. On the compliant
side, the realization identities and ergodic averaging for the selected policy
imply
\[
...
g_{i,\mathrm{rt}}^*
+
O\!\left(\eta+\delta+\frac1T\right).
\]
On the deviating side, the Bellman-$\Psi$ supermartingale yields
\[
\gamma_i^T\bigl(s,(\tau_i,(\sigma_{\eta,\delta,M_*})_{-i})\bigr)
\le
g_{i,\mathrm{rt}}^*
+
O\!\left(\eta+\frac1T\right).
\]
Subtracting the two estimates gives
\[
\gamma_i^T\bigl(s,(\tau_i,\sigma_{-i})\bigr)-\gamma_i^T(s,\sigma)
\le
A_\eta\eta + C_\eta\delta + \frac{K_\eta}{T}.
\]
Thus v25 keeps the $Z/\Psi$ machinery, but the final comparison is now the
standard optimality argument for the deviator's induced MDP.
\end{abstract}
...
\subsection{The v25 reframing}

Version~25 keeps the v10--v24 frozen-continuation, local-Nash,
...
and stagewise-$\Psi$ repairs, but it changes the interpretation of the final
benchmark. The issue exposed by Pass~112 is not the $Z/\Psi$ telescope itself.
The issue is to justify why the same scalar $g_{i,\mathrm{rt}}^*$ controls both
the compliant profile and every unilateral deviation.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The structural defect from the earlier passes is unchanged. The visit
functional
\[
\widetilde U_i(V)=U_i^{\mathrm{act}}(V)+\sum_{b\in B_C}I_V(b)\,v_{i,C}(b)
\]
cannot be telescoped across visits, and the raw nodewise bias
\[
\Phi_{i,t}:=h_{i,C_t}^*(X_t)
\]
cannot be telescoped across strict node transitions either. If play exits node
$C$ through $b$ and the next stage is routed to $D=C[b]$, then the Bellman
equation in $C$ credits $v_{i,C}(b)$ whereas the next stage starts from the
child-node bias $h_{i,D}^*(X_{\mathrm{in}})$. Those are different objects.

\item The missing object is still an explicit transition stock. Define
\[
\Psi_{i,t}:=h_{i,C_t}^*(X_t)+Z_{i,t},
\qquad
Z_{i,1}=0,
\]
and whenever stage $t$ exits $C$ through $b$ into $D=C[b]$, set
\[
Z_{i,t+1}-Z_{i,t}=v_{i,C}^{x^*}(b)-h_{i,D}^*(X_{t+1}).
\]
The increment of $Z$ exactly compensates for the jump from $h_{i,C}^*$ to
$h_{i,D}^*$.

\item Now fix a player $i$ and freeze the other players at
$(\sigma_{\eta,\delta,M_*})_{-i}$. Because the routing automaton and visit
controllers use only finite public memory, player $i$ faces a finite one-player
MDP whose state records the current routed node together with the current
internal/controller state. A unilateral deviation is exactly a policy in this
MDP, and the compliant behavior of player $i$ is the stationary policy induced
by the local kernels $\sigma_{i,C}^*$.

\item For this induced MDP, the nodewise Bellman certificates
$(g_{i,C}^*,h_{i,C}^*)$ are approximate local solutions of the average-reward
optimality equation with frozen continuation data. The stagewise inequality is
\[
\E\bigl[\Psi_{i,t+1}\mid \cF_t\bigr]
\le
\Psi_{i,t}+g_{i,C_t}^*-u_i^t+O(\eta),
\]
with equality on the support of the compliant local kernel. Thus $\Psi$ is the
supermartingale certificate for the deviator in the induced MDP.
...
\]
still belongs only to the compliant node data. It identifies the compliant
renewal benchmark inside node $C$, but it is not a deviator-routed identity and
cannot be transported along an arbitrary deviating path.

\item The fixed-point relation
\(v_{i,C}^{x^*}(b)=g_{i,C[b]}^*+O(\eta)\)
makes the gains $g_{i,C}^*$ dynamically consistent across the tree. This is the
rooted Bellman recursion for continuation values in the induced MDP.

\item Therefore $g_{i,\mathrm{rt}}^*$ is the value of the induced one-player
MDP started from the root, up to the explicit $O(\eta)$ approximation coming
from frozen continuation matching. It is not a scalar benchmark imported
directly from the multiplayer game.

\item Under the compliant profile, the selected stationary policy is
approximately optimal in that MDP, and the realization identities together with
ergodic averaging give
\[
\gamma_i^T(s,\sigma)
=
g_{i,\mathrm{rt}}^*+O\!\left(\eta+\delta+\frac1T\right).
\]
Under any unilateral deviation, the Bellman-$\Psi$ supermartingale gives
\[
\gamma_i^T\bigl(s,(\tau_i,\sigma_{-i})\bigr)
\le
g_{i,\mathrm{rt}}^*+O\!\left(\eta+\frac1T\right).
\]
Hence the regret is $O(\eta+\delta+1/T)$.
\end{enumerate}
...
stagewise potential drift lemma, the direct comparison lemma via the
root value of the induced MDP, and finally \Cref{thm:main}. The appendices
record the finite-MDP facts, including the induced-MDP comparison proposition,
and the stagewise summation lemma.
...
Version~25 keeps it local and performs the global telescope instead with the
stock-augmented stagewise potential
$\Psi_{i,t}=h_{i,C_t}^*(X_t)+Z_{i,t}$ defined later in
\Cref{sec:proof-main}. Thus $\widetilde U_i$ is not telescoped across visits,
and it is not part of the payoff criterion of the stochastic game.
...
\subsection{The induced one-player MDP and the stage-by-stage potential telescope}
...
$v_{i,C}^{x^*}(b)$ and the successor-node entry bias. In v25 the same process is
read as the relative-value certificate for the induced one-player MDP faced by a
deviator.
\end{remark}

\begin{lemma}[Direct comparison via the root value of the induced MDP]
\label{lem:direct-tail-comparison}
...
$A_\eta^{\mathrm{dir}},K_\eta^{\mathrm{dir}}<\infty$ such that
\begin{equation}
\label{eq:direct-tail-comparison}
\gamma_i^T\bigl(s,\varsigma^\tau\bigr)
-
\gamma_i^T\bigl(s,\varsigma^\sigma\bigr)
\le
A_\eta^{\mathrm{dir}}\eta + C_\eta\delta + \frac{K_\eta^{\mathrm{dir}}}{T}.
\end{equation}
\end{lemma}

\begin{proof}
Apply \Cref{prop:mdp-direct-comparison} to the compliant profile
$\varsigma^\sigma$ and the deviation profile $\varsigma^\tau$. That proposition compares the two profiles only through the root value of
the induced MDP, namely $g_{i,\mathrm{rt}}^*$, and yields
\[
\gamma_i^T\bigl(s,\varsigma^\tau\bigr)
-
\gamma_i^T\bigl(s,\varsigma^\sigma\bigr)
\le
2A_\eta^{\mathrm{root}}\eta + C_\eta\delta + \frac{2K_\eta^{\mathrm{root}}}{T}.
\]
After enlarging $A_\eta^{\mathrm{dir}}$ and $K_\eta^{\mathrm{dir}}$ to dominate the
right-hand constants, this is exactly \eqref{eq:direct-tail-comparison}.
\end{proof}

\begin{remark}[Why v25 uses the root MDP value]
The repaired argument is not that $g_{i,\mathrm{rt}}^*$ happens to be a common
scalar in two unrelated calculations. Once the other players are frozen, both
the compliant behavior and every unilateral deviation are policies in the same
finite one-player MDP. The identity
\(g_{i,C}^*=\alpha_{i,C}^*+\sum_b\beta_C^*(b)v_{i,C}^{x^*}(b)\)
remains a compliant realization identity. The deviator is bounded instead by the
Bellman-$\Psi$ supermartingale, which is the standard upper certificate for an
average-reward MDP. That is why the same scalar $g_{i,\mathrm{rt}}^*$
legitimately benchmarks both sides.
\end{remark}

Fix once and for all a constant
\[
A_\eta\ge 1+A_\eta^{\mathrm{dir}}.
\]
...
\Cref{thm:realization,lem:direct-tail-comparison}. Choose
$\delta>0$ such that
\[
...
\Cref{lem:direct-tail-comparison}. Choose $T_0$ so large that
\[
\frac{K_\eta}{T_0}\le \frac{\eps}{3}.
\]

Now fix $T\ge T_0$, an initial state $s$, a player $i$, and a unilateral
deviation $\tau_i$. By \Cref{lem:direct-tail-comparison},
\begin{equation}
\label{eq:direct-tail-comparison-proof}
...
under condition~\textnormal{(a)} and the induced-MDP comparison from
\Cref{lem:direct-tail-comparison}.

The conceptual point of v25 is the benchmark interpretation. The identity
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b)
\]
belongs to the compliant node data and remains useful only there. It does not,
by itself, control an arbitrary deviator-routed node sequence. The correct final
comparison is different: when player $i$ deviates, the other players continue to
play $(\sigma_{\eta,\delta,M_*})_{-i}$, so player $i$ faces a finite one-player
MDP with fixed environment.

In that induced MDP, the stock-augmented potential
\[
\Psi_{i,t}
=
h_{i,C_t}^*(X_t)+Z_{i,t},
\qquad
Z_{i,t+1}-Z_{i,t}=v_{i,C_t}^{x^*}(b)-h_{i,C_t[b]}^*(X_{t+1}),
\]
is the Bellman supermartingale certificate. The compliant policy is the
approximately optimal stationary policy selected by the fixed point, so its
long-run average payoff is $g_{i,\mathrm{rt}}^*+O(\eta+\delta+1/T)$. No unilateral
policy in the same induced MDP can exceed that root value by more than
$O(\eta+1/T)$. Once $g_{i,\mathrm{rt}}^*$ is read as the value of the deviator's
induced MDP, rather than as a scalar benchmark imported directly from the
multiplayer game, the regret estimate becomes the standard average-reward MDP
optimality argument.
\appendix



\section{Finite average-reward MDP facts and the induced root-MDP comparison}
\label{app:mdp}
...
background. The point specific to v25 is that when player $i$ deviates and the
other players keep the realized profile, player $i$ faces a finite routed MDP.
The Bellman certificates constructed in the main text are approximate optimality
certificates for that induced control problem.
...
\begin{proposition}[The routed unilateral-control problem is a finite MDP]
...
MDP. Its state records the current routed node, the current internal/controller
state, and the finite public memory of the frozen cap schedule. The stage reward
is the expected stage payoff against the frozen opponents, and the transition
kernel is the induced law of the next routed public-memory state. Every
unilateral deviation strategy $\tau_i$ determines a policy in this MDP, and the
compliant behavior of player $i$ is the stationary policy induced by the local
kernels $\sigma_{i,C}^*$.
\end{proposition}

\begin{proof}
By \Cref{prop:public-routing}\textnormal{(R4)}, the realized profile uses only
finite public memory. Once the nondeviators' mixed actions are frozen, the law
of the next routed public-memory state depends only on the current routed
...
finite average-reward MDP. Any unilateral deviation strategy induces a possibly
history-dependent policy of this MDP, and conversely the compliant local kernels
$\sigma_{i,C}^*$ define a stationary policy on the routed state space.
\end{proof}

\begin{proposition}[Two-sided comparison through the root value of the induced MDP]
\label{prop:mdp-direct-comparison}
...
$\varsigma^\tau:=(\tau_i,(\sigma_{\eta,\delta,M_*})_{-i})$. Write
\[
g_{i,\mathrm{rt}}^*:=g_{i,C_{\mathrm{rt}}}^*.
\]
Then $g_{i,\mathrm{rt}}^*$ is the root value of the induced one-player MDP from
\Cref{prop:induced-routed-mdp}, up to an $A_\eta^{\mathrm{root}}\eta$
approximation. More precisely, there exist finite constants
$A_\eta^{\mathrm{root}},K_\eta^{\mathrm{root}}<\infty$ such that for every horizon
$T$,
\begin{align*}
\gamma_i^T\bigl(s,\varsigma^\sigma\bigr)
&\ge
g_{i,\mathrm{rt}}^* - A_\eta^{\mathrm{root}}\eta - C_\eta\delta - \frac{K_\eta^{\mathrm{root}}}{T},
\\
\gamma_i^T\bigl(s,\varsigma^\tau\bigr)
&\le
g_{i,\mathrm{rt}}^* + A_\eta^{\mathrm{root}}\eta + \frac{K_\eta^{\mathrm{root}}}{T}.
\end{align*}
Consequently,
\[
\gamma_i^T\bigl(s,\varsigma^\tau\bigr)
...
2A_\eta^{\mathrm{root}}\eta + C_\eta\delta + \frac{2K_\eta^{\mathrm{root}}}{T}.
\]
In particular, the compliant and deviating profiles are compared only through the
root value of the induced one-player MDP; no identity of the form
$g-\alpha=\Delta Z$ is used along the deviator-routed path.
\end{proposition}

\begin{proof}
By \Cref{prop:induced-routed-mdp}, once the nondeviators are frozen at
$(\sigma_{\eta,\delta,M_*})_{-i}$, player $i$ faces a finite average-reward MDP.
Condition~\textnormal{(a)} makes every local node problem communicating, so
\Cref{prop:bias-unique} identifies $(g_{i,C}^*,h_{i,C}^*)$ as the normalized
optimality pair of the node MDP in every node $C$. The frozen continuation
constraints \eqref{eq:frozen-continuation} say that, when play exits $C$ through
$b$, the continuation certificate is the child value $g_{i,C[b]}^*$ up to an
$\eta$ error. Thus the family $g_{i,C}^*$ is dynamically consistent across the
rooted tree and satisfies, for every ordinary node $C$,
\begin{equation}
\label{eq:root-benchmark-recursion}
\left|
g_{i,C}^*
-
\alpha_{i,C}^*
...
\le \eta,
\end{equation}
because $\sum_{b\in B_C}\beta_C^*(b)\le 1$. For a degenerate terminal node $T$,
one has $g_{i,T}^*=w_i(T)=\alpha_{i,T}^*$, so the same recursion is exact. This
is the rooted Bellman recursion for the induced MDP, and it identifies
$g_{i,\mathrm{rt}}^*$ as the root MDP value up to the stated $O(\eta)$ error.

For a node $C$, let $\Gamma_{i,C}^{T,\sigma}$ denote the expected $T$-stage
average payoff of the continuation play obtained by starting the routed MDP in
node $C$ and then following the compliant policy, and let $\Gamma_{i,C}^{T,\tau}$
denote the corresponding supremum over unilateral policies of player $i$. We
prove by induction on the remaining depth $d_\eta(C)$ that, uniformly over entry
phase-states,
\begin{align}
\Gamma_{i,C}^{T,\sigma}
&\ge
g_{i,C}^* - A_\eta^{\mathrm{root}}\eta - C_\eta\delta - \frac{K_\eta^{\mathrm{root}}}{T},
\label{eq:root-compliant-node}
\\
\Gamma_{i,C}^{T,\tau}
&\le
g_{i,C}^* + A_\eta^{\mathrm{root}}\eta + \frac{K_\eta^{\mathrm{root}}}{T}.
\label{eq:root-deviator-node}
\end{align}

If $d_\eta(C)=1$, then there is no strict child below $C$. The compliant bound is
just the local realization identity from \Cref{thm:realization}\textnormal{(Q2)}
together with the bounded prelude/final-fragment term from
\Cref{thm:realization}\textnormal{(Q3)}. The deviator bound is the one-node
Bellman estimate from \Cref{lem:visit-bellman}\textnormal{(b)}, followed again by
\Cref{thm:realization}\textnormal{(Q3)}. Enlarging $K_\eta^{\mathrm{root}}$ fixes
the base case.

Assume \eqref{eq:root-compliant-node}--\eqref{eq:root-deviator-node} already hold
for all strict children of $C$. Under the compliant policy, each complete tail
visit $V$ in node $C$ satisfies the exact realization identities
\[
\E\bigl[U_i^{\mathrm{act}}(V;\sigma_{\eta,\delta,M_*})\mid \cF_{\mathrm{in}(V)}\bigr]
=
\alpha_{i,C}^*\,\E\bigl[\ell(V)\mid \cF_{\mathrm{in}(V)}\bigr],
\qquad
\E\bigl[I_V(b)\mid \cF_{\mathrm{in}(V)}\bigr]
=
\beta_C^*(b)\,\E\bigl[\ell(V)\mid \cF_{\mathrm{in}(V)}\bigr].
\]
If that visit exits through $b$, the continuation MDP starts in the child
$C[b]$; the induction hypothesis then contributes the child benchmark
$g_{i,C[b]}^*$ up to the same $O(\eta+\delta+1/T)$ remainder. Plugging these
continuation terms into the last display and using
\eqref{eq:root-benchmark-recursion} yields the compliant bound for $C$. This is
exactly the ergodic-average estimate for the approximately optimal stationary
policy of the induced MDP, written in routed-tree coordinates.

For a unilateral deviation, \Cref{lem:visit-bellman}\textnormal{(b)} gives a
stagewise Bellman upper bound inside node $C$, with the stock update
\eqref{eq:psi-stock-update} carrying the continuation certificate across a strict
exit. When a visit exits through $b$, \eqref{eq:frozen-continuation} replaces the
carried continuation value $v_{i,C}^{x^*}(b)$ by the child benchmark
$g_{i,C[b]}^*$ at cost at most $\eta$, and the induction hypothesis controls the
remaining play in that child. Thus the $\Psi$-adjusted payoff process is the
standard Bellman supermartingale for arbitrary policies in the induced MDP,
dominated by $g_{i,C}^*$ up to the explicit $A_\eta^{\mathrm{pot}}\eta$ drift term
and the bounded endpoint term of $\Psi$. Since every routed play has at most
$D_\eta-1$ strict child moves, all recursive continuation and endpoint
contributions are absorbed into one constant $K_\eta^{\mathrm{root}}$. This proves
the deviator bound for $C$.

Applying \eqref{eq:root-compliant-node} and \eqref{eq:root-deviator-node} to the
root node $C_{\mathrm{rt}}$ gives the two displayed inequalities of the
proposition. The final regret bound is obtained by subtraction.
\end{proof}
