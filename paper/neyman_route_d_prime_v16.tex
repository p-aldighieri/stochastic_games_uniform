...
Version~16 removes the last two expectation-level breaks from the downstream
argument. Execution never leaves the actual stochastic-game payoff model
\[
u_i(s_t,a_t)\in[0,1].
\]
...
routing automaton stays in a fixed node. For a compliant visit, the controller is
used only through expectation identities:
\[
\E[U_i^{\mathrm{act}}(V)] = \alpha_{i,C}\,\E[\ell(V)],
\qquad
\E[I_V(b)] = \beta_C(b)\,\E[\ell(V)].
\]
For a deviating visit, the Bellman certificate and optional stopping are applied
directly to the expected exit-corrected payoff
\[
\widetilde U_i(V)
=
U_i^{\mathrm{act}}(V)+\sum_{b\in B_C}I_V(b)\,v_{i,C}(b),
\]
never to a payoff ratio. This auxiliary functional is a stopping-time
bookkeeping device, not a stage-payoff modification of the game. Because the
rooted tree has finite depth, the total difference between the auxiliary and
actual payoffs over the whole horizon is $O_\eta(1)$.

The routing balance is now written entirely in expectation. The fixed-point
identity
\[
g_{i,C}
=
...
O(\eta)
\]
is multiplied by expected visit lengths and combined with
$\E[I_V(b)] = \beta_C(b)\E[\ell(V)]$; the rooted-tree telescoping is therefore
performed on expected exit counts, not on stationary rates pathwise. Summing the
visitwise Bellman inequalities yields a regret bound of the form
\[
...
\subsection{The v16 repair}

Version~16 keeps the v10-v15 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
expectation-bookkeeping, and finite-memory repairs, and it fixes the last two
fatal expectation-level breaks in the downstream proof.
...
\item For compliant play, the local controller lemma works directly with
$\E[U_i^{\mathrm{act}}(V)]$, $\E[\ell(V)]$, and $\E[I_V(b)]$. In particular,
\[
\E[U_i^{\mathrm{act}}(V)] = \alpha_{i,C}\,\E[\ell(V)],
\qquad
\E[I_V(b)] = \beta_C(b)\,\E[\ell(V)].
\]
No step passes from $U_i^{\mathrm{act}}(V)/\ell(V)$ to
$\E[U_i^{\mathrm{act}}(V)]/\E[\ell(V)]$.
\item The Bellman certificate is applied to the auxiliary
...
$I_V(b)$ records whether the visit exits through $b$. The deviator bound is an
optional-stopping inequality for $\E[\widetilde U_i(V)]$, again without any
ratio-of-expectations step.
\item The routing balance is written in expectation. The stationary exit rates
$\beta_C^*(b)$ are converted into expected exit counts through
\[
\E[I_V(b)] = \beta_C^*(b)\,\E[\ell(V)],
\]
and the rooted-tree telescoping is performed on those expected exit counts, not
on stationary weights pathwise.
\item The final regret estimate is therefore obtained by summing visitwise
expectation inequalities over the rooted tree, using the fixed-point balance and
the finite-depth bound, while the actual payoff criterion remains untouched.
\end{enumerate}
...
constant $K_\eta^{\mathrm{bal}}$ such that for every finite random family
$\mathcal V$ of complete tail visits compatible with the public routing
automaton and equipped, on each visit, with the compliant controller of the
corresponding node,
\begin{equation}
...
\begin{proof}
The fixed-point continuation condition \eqref{eq:frozen-continuation} and the
nodewise identity \eqref{eq:node-gain-identity} imply that, at every ordinary
node $C$,
\begin{equation}
...
|\theta_{i,C}|\le \eta.
\end{equation}
Let $V\in\mathcal V$ be a compliant visit in node $C$. By
\eqref{eq:visit-exit-realization},
\[
\E\bigl[I_V(b)\bigr]
=
\beta_C^*(b)\,\E\bigl[\ell(V)\bigr]
\qquad (b\in B_C).
\]
Multiplying \eqref{eq:routing-balance-node} by $\E[\ell(V)]$ therefore gives
\[
g_{i,C}^*\,\E[\ell(V)]
=
\alpha_{i,C}^*\,\E[\ell(V)]
+
\sum_{b\in B_C} g_{i,C[b]}^*\,\E[I_V(b)]
+
\theta_{i,C}\,\E[\ell(V)].
\]

Now sum this identity over the chronological visit family $\mathcal V$.
Because the routing automaton moves only to strict children, the actual exit
indicators $I_V(b)$ match child entries pathwise, and hence the terms
\[
\sum_{V\in\mathcal V}\sum_{b\in B_C} g_{i,C[b]}^*\,\E[I_V(b)]
\]
telescope in expectation down the rooted tree. The only unmatched terms come
from the initial root contribution and the terminal cutoff of the finite family.
Since the tree has finite depth, these boundary contributions are bounded by a
constant depending only on $\eta$ and the finite tree object; absorb them into
$K_\eta^{\mathrm{bal}}$. Rearranging the resulting expectation identity yields
...
The conceptual point of v16 is that the proof never leaves the actual payoff
model of the stochastic game and never passes through an invalid ratio-of-
expectations step. The realized play is always evaluated with the original stage
payoff array $u_i(s_t,a_t)\in[0,1]$. When a node visit exits, the routing
automaton moves immediately to the successor node and actual play resumes there
at the next stage. There is no cemetery phase and no artificial per-stage
continuation payoff.
...
to a node visit. For compliant play, the controller is used through exact
...
\E[I_V(b)] = \beta_C^*(b)\,\E[\ell(V)].
\]
For deviating play, the Bellman certificate and optional stopping control
$\E[\widetilde U_i(V)]$ directly. The global argument therefore proceeds by
decomposing the horizon into actual node visits, controlling each visit locally
through $\E[U]$, $\E[\ell]$, and $\E[I(b)]$, and then summing those expectation
inequalities through the routing balance of the fixed point.

This is exactly where v15 still failed. It treated a single-visit ratio
$U_i^{\mathrm{act}}(V)/\ell(V)$ as if one could pass to
$\E[U_i^{\mathrm{act}}(V)]/\E[\ell(V)]$, and it routed with stationary exit
rates $\beta_C^*(b)$ as though they were realized exit counts. Version~16 fixes
both points. Everything now telescopes only in expectation: the fixed-point
identity is multiplied by expected visit lengths, stationary exit rates are
converted to expected exit counts through
$\E[I_V(b)] = \beta_C^*(b)\E[\ell(V)]$, and the auxiliary/actual discrepancy
remains $O_\eta(1)$ because the rooted tree has finite depth.
