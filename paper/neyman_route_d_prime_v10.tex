...
Version~10 keeps the v9 repairs and fixes the one remaining bookkeeping
point from the conditional review: terminal-label leaf accounting. The
gain/continuation coordinates remain placed in depth-dependent compact boxes, the
finitely many prelude blocks before the tail scale $M_*$ remain peeled off into
the $K_\eta/T$ remainder, and terminal routing is now accounted for all the way
to the horizon. When a block exits to a terminal label $b$, the augmented block
payoff records the one-shot continuation value $w(b)$, no further blocks are
scheduled, and the remaining stages are played under the fixed terminal
continuation profile attached to $b$. Because terminal labels are leaves of the
finite universal tree, the resulting post-terminal contribution to horizon-$T$
regret is bounded by $D_\eta\|u\|_\infty/T$ and is absorbed into the
$K_\eta/T$ remainder. The bookkeeping kernel with restart through $\rho_C$ is
used only to define the stationary rate package
...
\subsection{The v10 repair}

Version~10 keeps the v9 frozen-continuation, local-Nash, bias-normalization,
augmented-kernel, routing, and renewal-reward repairs, and adds the final
terminal-label leaf-accounting fix from the conditional review.
...
to $b$.
\item Terminal labels are treated as leaves in the horizon accounting. If a
block exits at a terminal label $b$, then the augmented block reward records the
one-shot continuation value $w(b)$, no further blocks are scheduled, and the
remaining stages are played under the fixed terminal continuation profile
attached to $b$. Because terminal labels carry no further tree structure, the
resulting post-terminal contribution to horizon-$T$ regret is bounded by
$D_\eta\|u\|_\infty/T$ and is absorbed into the $K_\eta/T$ remainder term.
\end{enumerate}
...
used for all remaining stages after that exit; its payoff vector is denoted by
$w(b)\in[0,1]^N$.
...
terminal label, then $v_C^x(b)=w(b)$ is the payoff vector of the fixed terminal
continuation profile attached to that label. In the local Bellman equations this
terminal quantity is used as a one-shot payoff at the exit stage; the actual
remaining stages under the fixed terminal profile are accounted for globally in
Section~\ref{sec:realization}. Throughout this section $x$ is considered fixed.
...
If the block exits at a terminal label $b$, that second term is the one-shot
terminal value $w(b)$; the actual remaining stages are then handled by the
global routing rule from \Cref{prop:public-routing}.
...
exiting augmented block reward records the one-shot continuation value $w(b)$,
and play continues for all remaining stages according to the fixed terminal
continuation profile attached to $b$. Since terminal labels have no further tree
structure, no additional blocks are scheduled at that label. If no cemetery
state is visited, the next block starts immediately in the same node.
...
label, the exiting block records the one-shot continuation value $w(b)$, control
passes to the attached fixed terminal continuation profile for all remaining
stages, and no further block is scheduled. If no label is emitted, the
controller remains in the same node. The bound by $D_\eta$ follows from
finiteness of the tree and the fact that every cemetery-routing move to a child
strictly decreases the remaining depth. The memory requirements are finite by
\Cref{lem:frozen-memory}, and all relevant quantities are publicly observable.
\end{proof}

\begin{remark}[Terminal-label leaf accounting]
\label{rem:terminal-leaf-accounting}
Suppose a launched block exits at a cemetery label $b$ with $C[b]$ terminal.
Then the exiting augmented block reward already contains the one-shot
continuation value $w(b)$. After that exit the public strategy prescribes the
fixed terminal continuation profile attached to $b$ for every remaining stage of
the horizon and launches no further blocks. Because terminal labels are leaves
of the finite universal tree, a crude uniform bound gives at most $D_\eta$ such
leaf encounters along any play path. Consequently the post-terminal
contribution to horizon-$T$ regret is at most
\[
\frac{D_\eta\|u\|_\infty}{T},
\]
and this term is absorbed into the $K_\eta/T$ remainder.
\end{remark}
...
node of block $B$. The constant $K_\eta$ absorbs the frozen prelude, the
incomplete fragments, and the possible post-terminal leaf segment after an exit
to a terminal label.
\end{enumerate}
\end{theorem}
...
plus at most one initial incomplete fragment, at most one terminal incomplete
fragment, and possibly one post-terminal leaf segment after the last
terminal-label exit. Among the complete blocks, only finitely many belong to the
frozen prelude and have cap $<M_*$. Their combined contribution to the realized
payoff and to the weighted-gain sum is bounded by a finite constant depending
only on the frozen prelude, the tail cap $M_*$, and the finite tree object;
absorb that constant into $K_\eta$. Summing
...
If the final routed block exits to a terminal label $b$, then that exiting block
already contains the one-shot terminal value $w(b)$. After the exit no further
blocks are launched and the profile follows the fixed terminal continuation
profile attached to $b$ for the remaining stages. By
\Cref{rem:terminal-leaf-accounting}, the resulting post-terminal leaf segment
contributes at most $D_\eta\|u\|_\infty/T$ to the horizon-$T$ discrepancy.
...
these constants together with the terminal-leaf contribution into $K_\eta$
yields \eqref{eq:realization-tail-bound}.
\end{proof}
...
fragments, together with the possible post-terminal leaf segment after a
terminal-label exit, contribute at most a finite constant depending only on the
frozen prelude, the tail cap, the finite tree object, and $D_\eta\|u\|_\infty$.
Absorb all of these terms into a constant $K_\eta<\infty$. Choose $T_0$ so large
that
\[
...
terminal label $b$, then the one-shot continuation value $w(b)$ is already
included in that block's augmented reward, no further blocks are scheduled, and
the remaining stages are played under the fixed terminal continuation profile
attached to $b$. By \Cref{rem:terminal-leaf-accounting}, the resulting
post-terminal leaf contribution to horizon-$T$ regret is at most
$D_\eta\|u\|_\infty/T$, so it is absorbed into the same $K_\eta/T$ remainder.
The incomplete fragments are also already absorbed into $K_\eta$. We obtain
\[
...
The central conceptual distinction in v10 is the separation between bookkeeping
and execution together with the terminal-label leaf accounting at the horizon.
The restart kernel $\rho_C$ belongs only to the convex bookkeeping package; it
is not an executed transition after a cemetery hit. Execution takes place on the
genuine absorbing augmented kernel, blocks terminate at the first cemetery
state, and the relevant local identity is renewal-reward: the bookkeeping rates
determine $g_{i,C}^*$, while the executed controller contributes that gain
through total augmented reward divided by actual first-exit time. When a block
hits a terminal label, the exiting block carries the one-shot continuation value
$w(b)$, block scheduling stops, and the remaining fixed terminal segment is
absorbed into the $K_\eta/T$ remainder. With those distinctions in place, the
fixed-point domain is convex, the Bellman target matches the realized first-exit
block, and the routing semantics are well-defined.
...
\item the incomplete fragments outside the complete blocks, together with any
post-terminal leaf segment after block scheduling stops, contribute at most $K$
in expectation.
\end{enumerate}
Then
\[
