...
\title{\textbf{Uniform Equilibrium from an Augmented Metastable Tree\\via Gain Near-Constancy and Node-Visit Decomposition}}
...
Version~18 fixes the last remaining global-assembly break from the Pass~98
review. Execution never leaves the actual stochastic-game payoff model
\[
u_i(s_t,a_t)\in[0,1].
\]
...
routing automaton stays in a fixed node. The proof still keeps the compliant and
deviating profiles separate and sums only over each profile's own complete visit
family. The new point is global assembly. At a fixed point, the node gains satisfy
...
\alpha_{i,C}^*
+
\sum_{b\in B_C}\beta_C^*(b)\,g_{i,C[b]}^*
+
O(\eta),
\]
and a depth induction yields the \emph{gain near-constancy} estimate
\[
g_{i,C}^*
=
g_{i,\mathrm{root}}^*+O_\eta(D_\eta\eta)
\qquad (C\in\cT_\eta).
\]
Hence every weighted average of node gains is already a root benchmark:
for any compliant or deviating visit family,
\[
\sum_V g_{i,C(V)}^*\,\E[\ell(V)]
=
g_{i,\mathrm{root}}^*\sum_V \E[\ell(V)]
+
O_\eta(D_\eta\eta T).
\]
No semi-Markov occupation theorem is needed for the deviator. For compliant play
the visit process also has a standard semi-Markov interpretation, which recovers
the same benchmark asymptotically, but the finite-horizon proof only uses gain
near-constancy and the bounded auxiliary/actual discrepancy.

The final regret bound is of order
\[
...
\subsection{The v18 repair}

Version~18 keeps the v10-v17 frozen-continuation, local-Nash,
...
fixes the last remaining break from the Pass~98 review.
...
visit one by one.
\item The global assembly no longer tries to turn exit-rate weights into time
fractions. Instead it uses a new \emph{gain near-constancy} lemma: at a fixed
point, for every player $i$ and every node $C$,
\[
g_{i,C}^*=g_{i,\mathrm{root}}^*+O_\eta(D_\eta\eta).
\]
This follows by induction on the tree depth from the fixed-point recursion
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
O(\eta).
\]
\item Consequently, for \emph{any} visit family generated by the public routing
automaton, compliant or deviating, one has
\[
\sum_V g_{i,C(V)}^*\,\E[\ell(V)]
=
g_{i,\mathrm{root}}^*\sum_V \E[\ell(V)]
...
O_\eta(D_\eta\eta T).
\]
Thus the global benchmark is obtained from total duration alone; no semi-Markov
occupation theorem is needed on the deviator side.
\item For compliant play there is also a standard semi-Markov interpretation of
the visit process, which recovers the same benchmark asymptotically. The
finite-horizon proof below, however, only needs gain near-constancy plus the
uniform $O_\eta(1)$ bound on the auxiliary/actual discrepancy.
\item The stochastic-game payoff criterion remains the original horizon average
of actual stage payoffs. The auxiliary exit-corrected visit functional
\[
\widetilde U_i(V)=U_i^{\mathrm{act}}(V)+\sum_{b\in B_C} I_V(b)\,v_{i,C}(b)
\]
is still only a stopping-time certificate inside the Bellman argument.
\item Because the rooted tree has finite depth, the total auxiliary/actual
discrepancy over the whole horizon is $O_\eta(1)$ and the prelude/final-fragment
contribution is also $O_\eta(1)$. The final regret estimate is therefore of
order
\[
A_\eta\eta + C_\eta\delta + \frac{K_\eta}{T},
\]
where $A_\eta$ is the finite gain near-constancy constant used in
...
gain near-constancy lemma, the compliant total benchmark, the visitwise Bellman
bound, the deviator total benchmark, and finally \Cref{thm:main}. The
...
\subsection{Gain near-constancy and total benchmark assembly}

The remaining global step is to replace the node-dependent benchmarks
$g_{i,C(V)}^*\,\ell(V)$ by the root benchmark
$g_{i,\mathrm{root}}^*\,\ell(V)$. Version~18 does this through a tree-depth
induction, not through any attempt to identify node durations with exit-rate
weights.

\begin{lemma}[Gain near-constancy]
\label{lem:gain-near-constancy}
Fix a player $i$ and a fixed point $x^*\in\BR_\eta(x^*)$. Then there exists a
finite constant $A_\eta^{\mathrm{nc}}<\infty$ such that for every node
$C\in\cT_\eta$,
\[
|g_{i,C}^*-g_{i,\mathrm{root}}^*|
\le
A_\eta^{\mathrm{nc}}\,\eta.
\]
One may take $A_\eta^{\mathrm{nc}}=D_\eta$. Consequently, for every family of
nonnegative weights $(w_C)_{C\in\cT_\eta}$,
\begin{equation}
\label{eq:gain-average-bound}
\left|
\sum_{C\in\cT_\eta} g_{i,C}^*\,w_C
-
g_{i,\mathrm{root}}^*\sum_{C\in\cT_\eta} w_C
\right|
\le
A_\eta^{\mathrm{nc}}\,\eta\sum_{C\in\cT_\eta} w_C.
\end{equation}
\end{lemma}

\begin{proof}
At the fixed point, \Cref{thm:local-controller} gives the node identity
\[
g_{i,C}^*
=
\alpha_{i,C}^*
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b).
\]
If $D=C[b]$ is the child attached to boundary label $b$, the frozen-continuation
constraint \eqref{eq:frozen-continuation} at the fixed point gives
\[
|v_{i,C}^{x^*}(b)-g_{i,D}^*|\le \eta.
\]
Hence every ordinary node satisfies the approximate recursion
\begin{equation}
\label{eq:gain-recursion-near-constancy}
g_{i,C}^*
=
\alpha_{i,C}^*
+
...
|\rho_{i,C}|\le \eta.
\end{equation}
Define the subtree oscillation
\[
\operatorname{osc}_i(C)
:=
\max\{|g_{i,E}^*-g_{i,F}^*|: E,F \text{ belong to the subtree rooted at } C\}.
\]
If $C$ is degenerate terminal, then $\operatorname{osc}_i(C)=0$. For an ordinary
node, \eqref{eq:gain-recursion-near-constancy} shows that passing from a node to
its children can enlarge the gain oscillation by at most the $\eta$-slack in the
fixed-point recursion. Therefore
\[
\operatorname{osc}_i(C)
\le
\eta+\max_{b\in B_C}\operatorname{osc}_i(C[b]).
\]
Induction on the remaining depth $d_\eta(C)$ yields
\[
\operatorname{osc}_i(C)\le d_\eta(C)\,\eta.
\]
Applying this at the root gives
\[
|g_{i,C}^*-g_{i,\mathrm{root}}^*|
\le
D_\eta\,\eta
\qquad (C\in\cT_\eta),
\]
which proves the first claim. The weighted estimate
\eqref{eq:gain-average-bound} is immediate by multiplying the pointwise bound by
$w_C\ge 0$ and summing over $C$.
\end{proof}

Fix once and for all a constant
\[
A_\eta\ge 1+A_\eta^{\mathrm{nc}}.
\]
This single constant will absorb both the gain oscillation from
\Cref{lem:gain-near-constancy} and the additional one-visit $\eta$-slack from
\Cref{lem:visit-bellman}.

For a profile $\varsigma$ following the public visit-routing convention and a
horizon $T$, write
\[
L_T(\varsigma):=\sum_{V\in\mathcal V_T(\varsigma)}\ell(V)
\]
for the total length of the complete tail visits contained in $[1,T]$.
...
$\sigma_{\eta,\delta,M_*}$. There exists a finite constant
...
\begin{proof}
Write $\sigma:=\sigma_{\eta,\delta,M_*}$. For each node $C$, set
\[
L_{C,T}^\sigma
:=
\E\left[
\sum_{V\in\mathcal V_T(\sigma):\,C(V)=C}\ell(V)
\right].
\]
Applying \eqref{eq:visit-aux-realization} visit by visit and summing yields
\[
\E\left[
...
\ge
\sum_{C\in\cT_\eta}
(g_{i,C}^*-C_\eta\delta)\,L_{C,T}^\sigma.
\]
By \eqref{eq:gain-average-bound} with weights $w_C=L_{C,T}^\sigma$ and the
identity
\[
\sum_{C\in\cT_\eta}L_{C,T}^\sigma=\E\bigl[L_T(\sigma)\bigr],
\]
the right-hand side is at least
\[
g_{i,\mathrm{root}}^*\,\E\bigl[L_T(\sigma)\bigr]
-
(A_\eta\eta+C_\eta\delta)\,\E\bigl[L_T(\sigma)\bigr].
\]
Finally, \eqref{eq:aux-actual-discrepancy} gives
\[
\sum_{V\in\mathcal V_T(\sigma)}
U_i^{\mathrm{act}}(V;\sigma)
\ge
\sum_{V\in\mathcal V_T(\sigma)}
\widetilde U_i(V;\sigma)-V_\eta.
\]
Absorb $V_\eta$ into $K_\eta^{\mathrm{bal}}$ to obtain
\eqref{eq:routing-balance}.
\end{proof}

\begin{remark}[Semi-Markov interpretation of compliant play]
Under the compliant profile, the sequence of visited nodes together with their
sojourn times forms a finite semi-Markov chain on the tree nodes. The embedded
transition law is induced by the exit labels and the sojourn law at node $C$ is
the visit length produced by $\mathcal R_{C,L}^{\sigma_C^*}$. The standard
semi-Markov ergodic theorem therefore identifies the asymptotic time fraction of
each node. Combined with \Cref{lem:gain-near-constancy}, this yields the same
root benchmark asymptotically. The finite-horizon proof above does not require
that extra ergodic input.
\end{remark}
...
visit family. No compliant visit appears in the argument.
...
\right]
\le
\sum_{C\in\cT_\eta} g_{i,C}^*\,L_{C,T}^\tau
+
...
:=
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\tau):\,C(V)=C}\ell(V)
\right].
\]

By \eqref{eq:gain-average-bound} with weights $w_C=L_{C,T}^\tau$ and
\[
\sum_{C\in\cT_\eta}L_{C,T}^\tau=\E\bigl[L_T(\varsigma^\tau)\bigr],
\]
the first term on the right-hand side satisfies
\[
\sum_{C\in\cT_\eta} g_{i,C}^*\,L_{C,T}^\tau
\le
g_{i,\mathrm{root}}^*\,\E\bigl[L_T(\varsigma^\tau)\bigr]
+
A_\eta^{\mathrm{nc}}\eta\,\E\bigl[L_T(\varsigma^\tau)\bigr].
\]
Since $A_\eta\ge 1+A_\eta^{\mathrm{nc}}$, the preceding two displays imply
\[
\E\left[
\sum_{V\in\mathcal V_T(\varsigma^\tau)}
\widetilde U_i(V;\varsigma^\tau)
...
g_{i,\mathrm{root}}^*\,\E\bigl[L_T(\varsigma^\tau)\bigr]
+
A_\eta\eta\,\E\bigl[L_T(\varsigma^\tau)\bigr]
+
K_{\eta,1}^{\mathrm{dev}}.
\]
Finally, \eqref{eq:aux-actual-discrepancy} gives
...
\widetilde U_i(V;\varsigma^\tau).
\]
Absorb $K_{\eta,1}^{\mathrm{dev}}$ into $K_\eta^{\mathrm{dev}}$, and add the
harmless nonnegative term
$C_\eta\delta\,\E[L_T(\varsigma^\tau)]$ to obtain
...
remainder from \Cref{prop:deviation-total}, the telescoping endpoint remainder
from \Cref{lem:aggregation}, the total auxiliary/actual discrepancy from
\eqref{eq:aux-actual-discrepancy}, the finitely many prelude visits before the
tail cap $M_*$ is reached, and the final incomplete visit cut by the horizon.
...
\eqref{eq:compliant-root-lower} and \eqref{eq:deviation-root-upper}
imply
\[
...
under condition~\textnormal{(a)} and the fixed-point gain near-constancy
estimate.

The conceptual point of v18 is that the proof no longer tries to convert
exit-rate data into time-occupation data. The local visit estimates are always
of the form $g_{i,C(V)}^*\,\ell(V)$ on the actual visit family of a single
profile. The new gain near-constancy lemma shows that every node gain in the
fixed-point tree differs from the root gain by at most $O_\eta(D_\eta\eta)$.
Therefore any weighted average of node gains, under compliant routing or under
an arbitrary unilateral deviation, is already the root benchmark up to the same
error. This removes the fatal v17 use of exit-rate weights as proxies for node
time fractions.

Continuation values remain only inside the stopped Bellman equations. They
appear through the auxiliary exit-corrected visit functional
\eqref{eq:visit-aux-payoff}, which adds at most one continuation term to a node
visit. For compliant play, the controller yields
\[
\E\left[\sum_{V\in\mathcal V_T^\sigma}\widetilde U_i(V)\right]
\ge
g_{i,\mathrm{root}}^*\,T-O_\eta(A_\eta\eta T)-O_\eta(\delta T)-O_\eta(1),
\]
and the bounded auxiliary/actual discrepancy turns this into the actual-payoff
lower bound. For deviating play, the Bellman certificate and optional stopping
control each deviator visit separately and yield
\[
\E\left[\sum_{V\in\mathcal V_T^\tau}U_i^{\mathrm{act}}(V)\right]
\le
g_{i,\mathrm{root}}^*\,T+O_\eta(A_\eta\eta T)+O_\eta(\delta T)+O_\eta(1).
\]

A standard semi-Markov interpretation is still available on the compliant side:
the visit process has an embedded finite Markov chain on the tree nodes together
with node-dependent sojourn times, and the semi-Markov ergodic theorem recovers
the same long-run benchmark. But that asymptotic theorem is now optional. The
finite-horizon regret comparison closes already at the level of weighted visit
durations because the node gains are nearly constant across the whole tree.
