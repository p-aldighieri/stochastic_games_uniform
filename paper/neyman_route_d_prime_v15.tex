...
Version~15 removes the cemetery-phase payoff construction entirely. Execution
never leaves the actual stochastic-game payoff model
\[
u_i(s_t,a_t)\in[0,1].
\]
...
routing automaton stays in a fixed node. For each visit, the local controller
lemma controls the empirical occupation measure on the actual stages, hence also
the actual average payoff in that node. In parallel, the Bellman certificate
controls an auxiliary \emph{exit-corrected visit functional}, equal to the actual
visit payoff plus at most one terminal continuation term at the exit label. This
auxiliary functional is a stopping-time bookkeeping device, not a stage-payoff
modification of the game. Because the rooted tree has finite depth, the total
difference between the auxiliary and actual payoffs over the whole horizon is
$O_\eta(1)$.

Summing the visitwise Bellman inequalities and using the fixed-point
routing-balance identity yields a regret bound of the form
\[
...
\subsection{The v15 repair}

Version~15 keeps the v10-v14 frozen-continuation, local-Nash,
bias-normalization, augmented-kernel, routing, terminal-node,
renewal-reward, and finite-memory repairs, but it removes the only remaining
fatal mismatch between the proof and the stochastic-game payoff model.

\begin{enumerate}[label=\textnormal{(\arabic*)},leftmargin=2.4em]
\item The proof no longer assigns any artificial post-exit stage payoff. In
particular, the continuation value $v_C(b)$ attached to a boundary label is
\emph{not} used as a literal payoff on the stages following the exit.
\item A node visit ends at its actual first exit time or cap. If the visit exits
through label $b$, routing moves immediately to the successor node $C[b]$ and
actual play resumes there at the next stage.
\item The horizon-$T$ payoff is decomposed as a sum of \emph{actual} stage
payoffs over node visits. A node visit is a contiguous interval of stages during
which the public routing automaton stays in one node.
\item Inside a node visit, the local controller lemma controls the empirical
occupation measure and therefore the actual nodewise average payoff on the
executed stages. This part of the argument uses only the genuine stage-payoff
array $u_i\in[0,1]$.
\item The Bellman certificate is applied to an auxiliary
\emph{exit-corrected visit functional}
\[
\widetilde U_i(V)=U_i^{\mathrm{act}}(V)+\sum_{b\in B_C} I_V(b)\,v_{i,C}(b),
\]
where $U_i^{\mathrm{act}}(V)$ is the actual payoff earned during the visit and
$I_V(b)$ records whether the visit exits through $b$. This auxiliary quantity is
used only in the stopping-time comparison. It is not part of the game's payoff
definition.
\item The difference between auxiliary and actual payoffs is now controlled by a
finite-depth boundary term rather than by cancellation of profile-dependent
cemetery completions. Since every strict child move lowers depth, the total
number of such continuation certificates along a play path is uniformly bounded.
\item The final regret estimate is therefore obtained by summing visitwise
inequalities over the rooted tree, using the fixed-point routing balance and the
finite-depth bound, while the actual payoff criterion remains untouched.
\end{enumerate}
\subsection{Roadmap}
...
routing-balance identity, the visitwise Bellman bound, and finally
\Cref{thm:main}. The appendices record the finite-MDP facts and the final visit
...
\]
Hence \eqref{eq:controller-input-occ} holds with zero error at degenerate
terminal nodes.
\end{remark}
...
\end{equation}
This auxiliary quantity is used only inside the stopped Bellman comparison and
the renewal-reward identity. It is not part of the payoff criterion of the
stochastic game.
...
\left|
\frac{\E[U_{i,C,L}^{\mathrm{act}}]}{\E[\tau_{C,L}]}
-
\alpha_{i,C}^*
\right|
\le \frac{K_C}{\sqrt L},
...
$L\ge L_C(\delta)$, every launched visit of cap $L$ is $\delta$-accurate in the
sense of \eqref{eq:local-controller-occ}--\eqref{eq:local-controller-aux-payoff}.
\end{theorem}
...
For the actual payoff rate, observe that
\[
\frac{U_{i,C,L}^{\mathrm{act}}}{\tau_{C,L}}
=
\sum_{(\xi,a)\in\Omega_C}
\widehat\mu_{C,L}^{\mathrm{rate}}(\xi,a)\,u_i(\xi,a).
\]
Since $u_i(\xi,a)\in[0,1]$ for every $(\xi,a)$, one has
\[
\left|
\frac{U_{i,C,L}^{\mathrm{act}}}{\tau_{C,L}}
-
...
\right|
\le
\| \widehat\mu_{C,L}^{\mathrm{rate}}-\mu_C^*\|_1.
\]
Taking expectations and using \eqref{eq:local-controller-occ} gives
\eqref{eq:local-controller-actual-payoff} after enlarging $K_C$.

For the auxiliary functional,
\[
\frac{\widetilde U_{i,C,L}}{\tau_{C,L}}
=
\sum_{(\xi,a)\in\Omega_C}
\widehat\mu_{C,L}^{\mathrm{rate}}(\xi,a)\,u_i(\xi,a)
+
\sum_{b\in B_C}
\widehat\beta_{C,L}(b)\,v_{i,C}^{x^*}(b).
\]
Hence
\[
\left|
...
\le
(1+D_\eta)\,
\|(\widehat\mu_{C,L}^{\mathrm{rate}},\widehat\beta_{C,L})-\bar\mu_C^*\|_1.
\]
...
\sum_{b\in B_C}\beta_C^*(b)\,v_{i,C}^{x^*}(b).
\]
Substituting into the previous display and using
\eqref{eq:local-controller-occ} proves
...
For (Q2), \Cref{thm:local-controller} yields the conditional lower bounds
\eqref{eq:visit-actual-realization} and \eqref{eq:visit-aux-realization} after
enlarging $C_\eta$ uniformly over the finite tree.
...
\subsection{Routing balance and visitwise Bellman inequality}

The nodewise quantity $\alpha_{i,C}^*$ from \eqref{eq:alpha-node} is the actual
average payoff per stage generated by the bookkeeping occupation package in node
$C$. The Bellman gain $g_{i,C}^*$ is an auxiliary certificate level for the
stopped problem, related to $\alpha_{i,C}^*$ by
\eqref{eq:node-gain-identity}. The next proposition is the fixed-point balance
statement that makes the nodewise occupation decomposition global.

\begin{proposition}[Routing-balance identity]
\label{prop:routing-balance}
Fix a player $i$ and a fixed point $x^*\in\BR_\eta(x^*)$. There exists a finite
constant $K_\eta^{\mathrm{bal}}$ such that for every finite family
$\mathcal V$ of complete tail visits compatible with the public routing
automaton,
\begin{equation}
\label{eq:routing-balance}
\left|
\sum_{V\in\mathcal V}\ell(V)\,\alpha_{i,C(V)}^*
-
g_{i,\mathrm{root}}^*\sum_{V\in\mathcal V}\ell(V)
\right|
\le
K_\eta^{\mathrm{bal}} + \eta\sum_{V\in\mathcal V}\ell(V),
\end{equation}
where $C(V)$ is the node of the visit $V$ and $\mathrm{root}$ is the root node
of the universal $\eta$-tree.
\end{proposition}

\begin{proof}
The fixed-point continuation condition
\eqref{eq:frozen-continuation} and the nodewise identity
\eqref{eq:node-gain-identity} imply that, at every ordinary node $C$,
\begin{equation}
\label{eq:routing-balance-node}
g_{i,C}^*
=
\alpha_{i,C}^*
+
...
O(\eta).
\end{equation}
Now sum \eqref{eq:routing-balance-node} over the chronological visit family
$\mathcal V$. Every strict child move lowers the depth in the rooted tree, hence
the continuation terms telescope down the tree up to a finite remainder coming
from the initial root node, the finitely many depth changes, and the terminal
cutoff of the visit family. Because the tree is finite and every strict child
move can occur only once along a branch, the remainder is bounded by a constant
depending only on $\eta$ and the tree object. This yields
...
\begin{corollary}[Auxiliary visit regret]
\label{cor:visit-regret}
There exists a constant $C_\eta<\infty$ such that for every complete tail visit
$V$ with announced cap at least $M_*$ played in node $C$, every player $i$, and
every unilateral deviation $\tau_i$,
\begin{equation}
\label{eq:visit-regret-ineq}
\begin{aligned}
\E\left[
\widetilde U_i(V;\tau_i,\sigma_{-i})
-
\widetilde U_i(V;\sigma_{\eta,\delta,M_*})
\Bigm|\cF_{\mathrm{in}(V)}
\right]
&\le
(\eta+C_\eta\delta)\,\E\bigl[\ell(V)\mid\cF_{\mathrm{in}(V)}\bigr]
\\
&\quad+
h_{i,C}^*(X_{\mathrm{in}}(V))
-
...
\begin{proof}
Apply \Cref{lem:visit-bellman} to the deviator's auxiliary visit functional.
By \eqref{eq:visit-aux-realization}, the compliant controller launched from the
same entry phase-state and announced cap satisfies
\[
\E\bigl[\widetilde U_i(V;\sigma_{\eta,\delta,M_*})\mid\cF_{\mathrm{in}(V)}\bigr]
\ge
\bigl(g_{i,C}^*-C_\eta\delta\bigr)\,
\E\bigl[\ell(V)\mid\cF_{\mathrm{in}(V)}\bigr].
\]
Subtracting this lower bound from \eqref{eq:visit-bellman-ineq} yields
\eqref{eq:visit-regret-ineq}.
\end{proof}
...
Choose $\delta>0$ such that $C_\eta\delta\le \eps/6$, where $C_\eta$ is the
constant from \Cref{cor:visit-regret}. Since the tree is finite, there is a
...
Let $K_\eta<\infty$ absorb five finite effects: the telescoping endpoint
remainder from \Cref{lem:aggregation}, the routing-balance remainder from
\Cref{prop:routing-balance}, the total auxiliary/actual discrepancy from
...
\mathcal V_T^{\tau}
\]
be the complete tail visits contained in $[1,T]$ under the deviating profile
$(\tau_i,\sigma^\eps_{-i})$, listed in chronological order. Apply
\Cref{cor:visit-regret} visit by visit and sum. Within each maximal run of
consecutive visits that stay in the same node, the bias terms telescope. Every
time the node changes, the play moves to a strict child, and this can happen at
most $D_\eta$ times. Therefore \Cref{lem:aggregation}, applied with visit
lengths $\ell(V)$, yields
\begin{equation}
\label{eq:aug-regret-aggregation}
\frac1T\E\left[
\sum_{V\in\mathcal V_T^{\tau}}
\bigl(
\widetilde U_i(V;\tau_i,\sigma^\eps_{-i})
-
\widetilde U_i(V;\sigma^\eps)
\bigr)
\right]
\le
\eta + C_\eta\delta + \frac{K_\eta}{T}.
...
Next compare the compliant actual payoff to the root gain. Let
\[
\mathcal V_T^{\sigma}
\]
be the complete tail visits contained in $[1,T]$ under the compliant profile
$\sigma^\eps$. By \eqref{eq:visit-actual-realization},
\[
\E\left[
...
\E\left[
\sum_{V\in\mathcal V_T^{\sigma}}
(\alpha_{i,C(V)}^*-C_\eta\delta)\,\ell(V)
\right].
\]
Applying \Cref{prop:routing-balance} to the compliant visit family gives
\begin{equation}
...
\right]
\ge
g_{i,\mathrm{root}}^* - \eta - C_\eta\delta - \frac{K_\eta}{T}.
\end{equation}

For the deviating family $\mathcal V_T^{\tau}$, combine
\eqref{eq:aug-regret-aggregation} with
\eqref{eq:visit-aux-realization}, \Cref{prop:routing-balance}, and the auxiliary
correction bound \eqref{eq:aux-actual-discrepancy}. This yields
\begin{equation}
...
for both the compliant and deviating profiles. Enlarging $K_\eta$ once if
necessary, \eqref{eq:compliant-root-lower} and \eqref{eq:deviation-root-upper}
imply
\[
...
under condition~\textnormal{(a)} and the fixed-point routing-balance identity.

The conceptual point of v15 is that the proof never leaves the actual payoff
model of the stochastic game. The realized play is always evaluated with the
original stage payoff array $u_i(s_t,a_t)\in[0,1]$. When a node visit exits, the
routing automaton moves immediately to the successor node and actual play resumes
there at the next stage. There is no cemetery phase and no artificial per-stage
continuation payoff.
...
to a node visit. Because the rooted tree has finite depth, the total difference
between this auxiliary functional and the actual payoff along a whole horizon is
uniformly bounded. The global argument therefore proceeds by decomposing the
horizon into actual node visits, controlling each visit locally by occupation
measures and Bellman certificates, and then summing those visitwise inequalities
through the routing balance of the fixed point.

This is exactly the point at which the v14 cemetery construction failed. In v14,
the continuation value was treated as a literal post-exit stage payoff on the
remaining portion of a scheduled block. That left the actual payoff model and
required cancellation of profile-dependent cemetery terms. Version~15 removes
that step completely. All continuation terms are now auxiliary and one-shot, and
the only payoffs that enter the stochastic-game criterion are the original
executed stage payoffs.
\appendix
